...
2. **Release check** (reworked per public PR #1519, @erf1nd0r — the old "version diff" compared the payload's version against the marker that same payload wrote, so it always said "already current" and never fetched) — the skill carries no version field and there is no plugin manifest; versioning lives at the distribution layer, so BOTH sides of the diff must come from it:
   ...
   Never diff the on-disk payload's version against the install marker — the payload is what wrote the marker, so that comparison always says "already current" and the update never fetches anything. `Tools/VersionDiff.ts` does exactly that and is therefore NOT part of this workflow; it remains on disk with no caller. Verified by running it: with the marker copied from the payload (what `DeployCore` does on every install) it returns `verdict: already-current` and never consults the distribution layer, so a newer release upstream is undetectable.
3. **Re-overlay system** — re-copy the system templates (CLAUDE, system prompt, `settings.system.json` minus hooks), and overwrite `<configRoot>/LIFEOS/VERSION` with the payload's version — the copyMissing deploys never touch an existing marker, and a stale marker re-trips step 2 forever. These are system-owned and safe to overwrite.
   ...
   The `LIFEOS/` runtime and skills library still deploy via `bun Tools/DeployCore.ts` `copyMissing`, which ADDS only files absent at the destination and NEVER overwrites a populated one. So a version's genuinely new system files (a new doc, a new tool) arrive, but **content changes to a system file that already exists are NOT delivered here** — that path is a backup + re-install (see the Rule below). The system templates named above are the deliberate exception.
