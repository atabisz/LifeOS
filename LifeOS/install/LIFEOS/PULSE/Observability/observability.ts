...
async function handleTelosOverview(): Promise<Response> {
  try {
    ...
    return Response.json({
      ...
      missions: missionsFull,
      goals,
      // Upstream's live builders, not the fork's TELOS-file parsers. Both sides
      // replaced base's `metrics: null` / `projects: null` / `team: null` with real
      // data, from different sources — this is a genuine either/or, not a union.
      // Upstream wins on two grounds: buildStatusMetrics/buildProjectsFromStatus/
      // buildTeamFromUserFiles read live state (work registry, Bunker monitor, USER
      // identity files), and they also fill `team`, which the fork left null. The
      // fork's parseMetrics/parseProjects read TELOS/METRICS.md and PROJECTS.md,
      // which on a stock install hold only `(sample)` stub entries — and neither
      // parser screens them (the SAMPLE_ENTRY_RE guard added later is applied only
      // in the personalization counter), so that path served sample rows to the
      // dashboard as though they were real. `stranded` still consumes the fork's
      // parsed `projects`, so nothing here goes dead.
      metrics: liveMetrics,
      challenges,
      strategies,
