...
async function main() {
  ...
  for (const job of config.jobs) {
    ...
    //
    // isAbsolute, not startsWith("/"), for the same reason as dashboardDir above:
    // the POSIX literal is false for `C:\...`, so on Windows an absolute script
    // path was joined onto PULSE_DIR, the existsSync failed, and the preflight
    // disabled a perfectly healthy cron job. Both halves are load-bearing — the
    // "~/" arm is checked first because a tilde path is not isAbsolute either.
    const resolveRef = (p: string): string => {
      // Both halves are load-bearing and each side fixed only one: homedir() for
      // the HOME-unset case (public issue #1729, @umair-a11y — `?? ""` made a
      // "~/x" path resolve to a relative "x" under cwd), and isAbsolute() for the
      // win32 case (`startsWith("/")` is false for `D:\...`, so an absolute path
      // on another drive got joined onto PULSE_DIR).
