...
function main(): void {
  ...
  // "LifeOS", not "LIFEOS": this is a filesystem lookup, not a framework token, and
  // the directory is spelled `skills/LifeOS` in both trees. The fork's PAI→LIFEOS
  // rename sweep upcased the string here, which made the check match nothing and
  // silently report the skill absent on every install that had it.
