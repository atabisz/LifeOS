...
### Operational Rules
- Writing style. For any document, prose, email, note, or doc written for {PRINCIPAL.NAME}, follow `~/.claude/alex-work-writing-style.md` by default — concise, plain copulas, named sources, varied sentence length, hyphens over em dashes, straight quotes, sentence-case headings; avoid AI tells (inflated significance, promotional voice, forced rule-of-three, "Challenges and Future" template). Run that file's rewrite checklist on longer/formal pieces. Master copy lives in the A2 vault at `raw/alex-work-writing-style.md`; re-copy here when it changes.
...
- Signed commits always. Every commit and tag MUST be SSH-signed with `~/.ssh/id_ed25519_signing.pub` as `alex@tabisz.org`. The machine is configured globally (`gpg.format=ssh`, `commit.gpgsign=true`, `tag.gpgsign=true`) so signing is automatic — just run `git commit`/`git tag` normally. NEVER pass `--no-gpg-sign` or `-c commit.gpgsign=false`, and never disable signing to "get past" an error. If a repo has a local override that turns signing off or points at the wrong key/email, fix that repo's config before committing. After committing, verify with `git log --show-signature -1` and confirm a good signature for `alex@tabisz.org`; treat unsigned/unverified as a failed step and STOP. If signing itself fails (key missing, passphrase prompt, agent error), STOP and report — do not commit unsigned. Do NOT set a global `core.hooksPath` to enforce this: `~/.claude` already routes hooks to its machine-local ggshield pre-commit path (Windows: `C:\ProgramData\git\hooks`; macOS/Linux: the local ggshield install path) and other repos use their own `.git/hooks` — a global override would silently break them.
