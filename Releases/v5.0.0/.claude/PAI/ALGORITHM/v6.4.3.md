...
| **F: Provenance/Reproducibility (NEW v6.4.1)** | Any task that will mutate state or produce a durable artifact (code edit, commit, deploy, doc write, benchmark, security change) | **Establish ground truth BEFORE the first mutating action, not at VERIFY.** Probe the environment facts the work depends on — git-tracked status, secret-scan posture, build/tool variant (e.g. Framework-MSBuild vs dotnet), harness continue-vs-abort behaviour, instructions/provenance hash. Capture them in `## Decisions` so the run is reproducible and the artifact's provenance is recorded at the outset. |
...
| **H: Deletion-Risk Census (NEW v6.4.3)** | Deleting, pruning, moving, or renaming a code symbol (function, class, type, interface, const, export) or module — any change premised on "nothing uses this" | **A grep name-count is NOT a usage census.** A symbol can be live through union-type membership, internal references, or test-only usage that a count of external importers misses. Before the delete/prune/move ISC lands, run `bun ~/.claude/PAI/TOOLS/SymbolCensus.ts <symbol> [root]` and act on the classified verdict (LIVE / REVIEW / LIKELY-SAFE-TO-REMOVE), not on a count. The tool is read-only and fails safe — treat anything other than `LIKELY-SAFE-TO-REMOVE` as still-live until proven otherwise. |
...
- **Provenance-at-OBSERVE (Preflight Gate F, NEW v6.4.1)** — establish environment ground truth (git-tracked status, secret-scan posture, build/tool variant, harness behaviour, provenance hash) BEFORE the first mutating action, not at VERIFY. Record it in `## Decisions` so the run is reproducible. Provenance checks belong in preflight; doing them postflight is the recurring "should have checked earlier" miss.
- **Deletion-Risk Census (Preflight Gate H, NEW v6.4.3)** — before deleting/pruning/moving/renaming a code symbol or module on the premise "nothing uses this," run `bun ~/.claude/PAI/TOOLS/SymbolCensus.ts <symbol> [root]` and act on the classified verdict, not a grep name-count. A symbol is live through union-type membership, internal references, or test-only usage that a count misses. The tool is read-only and fails safe; treat anything other than `LIKELY-SAFE-TO-REMOVE` as still-live. This is a tool/delegation-class capability, NOT a thinking capability — it does not enter the v6.3.0 closed thinking enumeration.
