...
| **H: Deletion-Risk Census (NEW v6.4.3; OBSERVE-timing + shared-model extension v6.4.4)** | Deleting, pruning, moving, or renaming a code symbol (function, class, type, interface, const, export) or module — any change premised on "nothing uses this"; OR **loosening a SHARED model/type/contract** (widening a field, relaxing a required constraint, broadening a union, exposing an internal type) | **A grep name-count is NOT a usage census.** A symbol can be live through union-type membership, internal references, or test-only usage that a count of external importers misses. Before the delete/prune/move ISC lands, run `bun ~/.claude/PAI/TOOLS/SymbolCensus.ts <symbol> [root]` (or `--symbols a,b,c` for a whole-module prune — one keep/cut table for N symbols at once) and act on the classified verdict (LIVE / REVIEW / LIKELY-SAFE-TO-REMOVE), not on a count. The tool is read-only and fails safe — treat anything other than `LIKELY-SAFE-TO-REMOVE` as still-live until proven otherwise. **Run the census at OBSERVE recon, not at the mutating step (NEW v6.4.4):** the delete-vs-retain split and dependents map are ground-truth facts that belong with the other preflight checks — discovering them mid-deletion costs scope corrections (reflections 27, 28). Record the keep/cut + dependents map in `## Decisions` before any mutation. **Shared-model blast-radius BEFORE loosening (NEW v6.4.4):** when the change loosens a shared model/contract, enumerate every consumer/emitter (route, serializer, client, guard, served payload) at OBSERVE — a loosened shared type leaks through surfaces a symbol-name census alone won't show (reflection 35 — a `CreateSessionRequest` REST leak Cato caught that a before-loosening blast-radius would have surfaced at recon). |
