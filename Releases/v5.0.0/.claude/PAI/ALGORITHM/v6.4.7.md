...
```
♻︎ Entering the PAI ALGORITHM… (v6.4.7) ═════════════
...
**Why:** the recurring miss is *edit-first, discover-the-break-after* — a dispatch contract collapsed by an `elif` merge, a positional rule reference silently orphaned by a renumber, an invisible regression caught only when a later lint/test/advisor pass happened to run (reflections 10, 15, 18, 22). Capturing the contract before the edit makes the break visible at the edit, not at VERIFY. This is the general-edit complement to the Security-Surface Edit Gate's proof-of-fire (which is a HARD, security-only *precondition*); this reflex is the cheap default for ordinary code and doctrine edits and is not block-on-fail.
