---
name: ISA
description: "Owns the Ideal State Artifact (ISA) — the universal primitive holding the articulated ideal state of any project, task, or work session as a hard-to-vary explanation. One document that articulates the ideal state, drives the build, verifies it, and records the evolution of understanding. Five workflows: Scaffold (fresh ISA from a prompt at an effort tier), Interview (adaptive Q&A to deepen sections), CheckCompleteness (score against the tier completeness gate, report gaps), Reconcile (deterministic merge of an ephemeral feature-file excerpt back into the master ISA, keyed on stable ISC IDs), Seed (bootstrap from a repo's README and structure). Locked twelve-section order: Problem, Vision, Out of Scope, Principles, Constraints, Goal, Criteria, Test Strategy, Features, Decisions, Changelog, Verification. Invoked automatically by the Algorithm at OBSERVE for any non-trivial task, or directly to scaffold/audit an ISA. USE WHEN a prompt mentions ideal state, ISA, ISC, ideal state criteria, project specification, hill-climb on a task, articulating done, scaffolding or auditing an ISA, reconciling a feature file back to a master ISA, or seeding an ISA from a project. NOT FOR creating new skills (use CreateSkill), running the Algorithm itself (it invokes this skill), or retrospective write-ups like postmortems or decision logs (the ISA is a commitment-time scaffold, not after-the-fact)."
effort: medium
---
