...
- **Runtime: pending** — a future PAI/TOOLS/LoopRunner.ts tool is the next ISA. Until it ships, `/loop` and `/loop --autoresearch` continue to operate on the current same-session-accumulating substrate.
...
A new PAI/TOOLS/LoopRunner.ts (planned, not yet on disk) will be the supervisor:
...
- LoopRunner.ts (pending): PAI/TOOLS/LoopRunner.ts — next ISA, not yet on disk
