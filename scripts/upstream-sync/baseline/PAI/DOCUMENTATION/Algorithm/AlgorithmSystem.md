...
**Spec location:** `PAI/ALGORITHM/v6.19.0.md` (canonical pointer: `PAI/ALGORITHM/LATEST`)
...
> Canonical doctrine lives in `PAI/ALGORITHM/v6.19.0.md` (or follow `PAI/ALGORITHM/LATEST`). This summary is convenience-only — when in doubt, read the spec.
...
**Canonical reference: `PAI/ALGORITHM/modes/README.md`** — that directory holds one file per mode plus the canonical six-tab taxonomy table. Read it for the authoritative answer to "what modes exist." Per-mode doctrine lives in `modes/iterate.md`, `modes/optimize.md`, `modes/ideate.md`, `modes/loop.md`, `modes/native.md`.

**Pulse metadata surface — `PAI/DOCUMENTATION/Pulse/PulseMetadata.md`** — catalog of all badges, strips, and panels that surface mode + ISA state in the Pulse UI. Includes the v2.10 frontmatter additions: `response_mode`, `algorithm_mode`, `current_state`, `ideal_state`, `capabilities_invoked`. JourneyStrip is the headline new visualization — surfaces the current-state → ideal-state journey with ISC progress dots between the endpoints.
...
These are the six tabs in the Pulse Agents page (`PAI/PULSE/Observability/src/app/agents/page.tsx` lines 23-30):
...
Full detail and resolution order: `PAI/ALGORITHM/mode-detection.md` § "Parameter Detection".
...
| **identity** | Principal/DA identity update — `PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md` or `DA_IDENTITY.md` |
| **doctrine** | An Algorithm- or LifeOS-level principle — proposed for `PAI/ALGORITHM/v{NEXT}.md` |
...
Full ISA format spec: `PAI/DOCUMENTATION/ISA/IsaFormat.md`. ISA skill: `skills/ISA/SKILL.md`.
...
| File | Purpose |
|------|---------|
| `PAI/ALGORITHM/v6.19.0.md` | Full execution spec — the definitive Algorithm reference |
...
| `PAI/DOCUMENTATION/ISA/IsaFormat.md` | ISA format specification |
...
*Full execution spec: `PAI/ALGORITHM/v6.19.0.md` (or `PAI/ALGORITHM/LATEST`) | ISA format: `PAI/DOCUMENTATION/ISA/IsaFormat.md` | ISA skill: `skills/ISA/SKILL.md`*
