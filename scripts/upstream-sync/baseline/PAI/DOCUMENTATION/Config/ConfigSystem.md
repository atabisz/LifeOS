...
├── PAI/PAI_SYSTEM_PROMPT.md                     # SYSTEM constitutional rules (public-safe)
├── PAI/TOOLS/PaiConfig.ts                       # SYSTEM typed loader (INTERFACE)
├── PAI/TOOLS/MergeSettings.ts                   # SYSTEM merge driver (INTERFACE)
└── PAI/USER → ~/.config/PAI/USER                # Symlink to USER tree

~/.config/PAI/USER/                              # USER tree (private USER-data repo, symlinked)
├── CONFIG/
│   ├── settings.user.json                       # USER settings overlay
│   ├── OPERATIONAL_RULES.md                     # USER operational rules (@-imported)
│   └── PAI_CONFIG.toml                          # USER typed-config implementation
...
| `PAI/USER/CONFIG/settings.user.json` | USER | Per-principal overlay: identity values, voice IDs, principal name + timezone, custom `spinnerVerbs` + `spinnerTipsOverride`, env overrides. Deep-merged into system defaults; USER always wins. |
...
| `PAI/PAI_SYSTEM_PROMPT.md` | SYSTEM | Constitutional rules (system prompt layer, loaded via `--append-system-prompt-file`). |
| `PAI/USER/CONFIG/OPERATIONAL_RULES.md` | USER | Per-principal operational rules: repo conventions, env paths, vendor-specific doctrine (Cloudflare token, deploy semantics). `@`-imported directly from `CLAUDE.md` — CC does not follow transitive `@`-imports. |
| `PAI/USER/CONFIG/PAI_CONFIG.toml` | USER | Typed user config (identity name/pronunciation/timezone, integration credentials, paths). Read via `PaiConfig.load()`. |
| `PAI/TOOLS/PaiConfig.ts` | SYSTEM (INTERFACE) | Typed loader. Returns a strongly-typed object; the schema is the SYSTEM↔USER contract. |
| `PAI/TOOLS/MergeSettings.ts` | SYSTEM | Deep-merge driver that produces `settings.json` from `settings.system.json` + `settings.user.json` at SessionStart. |
| `PAI/USER/INTEGRATIONS/{homebridge,unifi,airgradient}.yaml` | USER | Per-integration credentials/endpoints for private `_*` skills. |
| `PAI/USER/WORK/config.yaml` | USER (optional) | `WORK.REPO` for the Work System. |

## How it works at runtime

1. **SessionStart** — `MergeSettings.ts` reads `settings.system.json` + `PAI/USER/CONFIG/settings.user.json`, deep-merges (USER wins on conflict), writes `settings.json`. This file is what Claude Code reads for hooks, permissions, env, identity.
...
3. **Skills** — private `_*` skills that need credentials/integration data read `PAI_CONFIG.toml` via `PaiConfig.load()` (e.g. `_HOMEBRIDGE` reads Homebridge token; `_NETWORK` reads UniFi creds).
...
- **Identity / voice / principal name / per-machine integrations** → edit `PAI/USER/CONFIG/settings.user.json` (USER overlay) or `PAI_CONFIG.toml`. Takes effect next SessionStart.
...
- **Constitutional rules** → edit `PAI/PAI_SYSTEM_PROMPT.md`. Takes effect next session.
...
- `~/.config/PAI/USER/` → `<your-username>/<your-user-data-repo>` (PRIVATE GitHub)

`~/.claude/.git/hooks/pre-push` auto-commits and pushes `~/.config/PAI/USER/` before each push from `~/.claude/`, so the two repos stay in sync structurally. A "kai update" / "push both repos" workflow wraps this with four boundary gates:
...
- **Phase C (2026-05-22)** — `CLAUDE.md` becomes thin router with direct `@`-imports of USER identity files. `OPERATIONAL_RULES.md` created in `PAI/USER/CONFIG/`. `CLAUDE.user.md` sidecar created and then deleted 2026-05-23 (merged back into `CLAUDE.md`) because CC doesn't follow transitive `@`-imports.
- **Phase E (2026-05-22)** — `SystemFileGuard.hook.ts` runtime write-time enforcement begins.
- **Phase F (2026-05-22)** — `PaiConfig.ts` typed loader + `PAI_CONFIG.toml` populated. Private skills migrated from hardcoded credentials to `PaiConfig.load()`.
- **Phase G (2026-05-22→23)** — separate private GitHub repo created for USER data. `~/.claude/PAI/USER` becomes symlink to `~/.config/PAI/USER`. Pre-push hook installed for two-repo sync. A two-repo-push workflow ("update the kai repo" / "push both repos") ships in the private `<your-release-skill>` skill with four boundary gates.
- **Phase H (deferred)** — PR-time `DenyListCheck` via GitHub Actions; community v5→v6 migration tool.

## Pre-v6.0 history

The pre-Phase-A model used `PAI_CONFIG.yaml` as a credentials store and a single `settings.json` directly edited by hand. There was no system/user boundary enforced at write time; defenses were release-time only. That model is retired — the contemporary architecture is the split + symlink + `PaiConfig.ts` described above. The release tooling preserves a `settings.public.json` template for the public release path but the live system reads only from the split files.

## Cross-references

- System/user boundary contract: `PAI/DOCUMENTATION/SystemUserBoundary.md`
- Master architecture: `PAI/DOCUMENTATION/LifeosSystemArchitecture.md`
- _LIFEOS skill workflows: `skills/_LIFEOS/SKILL.md` (UpdateKaiRepo, CreateShadowRelease, CreateRelease)
- Hook system: `PAI/DOCUMENTATION/Hooks/HookSystem.md` (SystemFileGuard, MergeSettings invocation)
