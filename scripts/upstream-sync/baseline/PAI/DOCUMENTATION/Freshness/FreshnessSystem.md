...
- `PAI/USER/DIGITAL_ASSISTANT/DA_IDENTITY.md` — the DA's voice, personality, relationship pact
- `PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md` — the principal's identity, role, worldview
- `PAI/USER/PROJECTS.md` — project registry + routing aliases
- `PAI/PAI_SYSTEM_PROMPT.md` — constitutional rules
- `PAI/USER/TELOS/PRINCIPAL_TELOS.md` — TELOS summary (auto-generated)
- `PAI/DOCUMENTATION/ARCHITECTURE_SUMMARY.md` — subsystem index (auto-generated)

Plus the underlying `PAI/USER/TELOS/TELOS.md` whose sections drive `PRINCIPAL_TELOS.md` and the per-section `ContextCheckin` workflow.
...
convention: pai-freshness-v1
derived_from: PAI/USER/TELOS/TELOS.md
generator: PAI/TOOLS/GenerateTelosSummary.ts
---
```
...
Single source: `~/.claude/PAI/TOOLS/TelosFreshness.ts` (named for historical reasons; covers all constitutional files).
...
bun ~/.claude/PAI/TOOLS/TelosFreshness.ts
bun ~/.claude/PAI/TOOLS/TelosFreshness.ts --json
bun ~/.claude/PAI/TOOLS/TelosFreshness.ts --bump goals

# Multi-file constitutional freshness
bun ~/.claude/PAI/TOOLS/TelosFreshness.ts context
bun ~/.claude/PAI/TOOLS/TelosFreshness.ts context --json

# Content quality audit (read-only)
bun ~/.claude/PAI/TOOLS/ContextAudit.ts
bun ~/.claude/PAI/TOOLS/ContextAudit.ts --json
```

## Pulse routes

Pulse module at `~/.claude/PAI/PULSE/modules/telos.ts` exposes the same data over HTTP:
...
The statusline FRESH line cannot afford a network call on every refresh (1s interval, blocking on Pulse-down would freeze rendering). `PAI/TOOLS/FreshnessCache.ts` writes a tiny mirror of `/api/freshness/summary` to a private file the statusline reads directly with `jq`.

```
~/.claude/PAI/USER/CACHE/freshness.json
```
...
3. **SessionStart hook** — `bun PAI/TOOLS/FreshnessCache.ts --quiet` runs async at session start, catching age-progression grade flips (a 28-day file with a 30-day threshold ages to 32 days and flips B→F with no bump event).
4. **CLI** — `bun PAI/TOOLS/FreshnessCache.ts` (or `--rebuild`) for manual rebuild; `--print` to dump JSON without writing; `--quiet` for silent success.

**Read path** — `PAI/PAI_StatusLine.sh` reads the cache file directly:

```bash
_FRESH_CACHE="$CLAUDE_HOME/PAI/USER/CACHE/freshness.json"
...
The autonomic memory loop (2026-05-22, see `PAI/DOCUMENTATION/Memory/MemorySystem.md`) uses a parallel but separate freshness mechanism for the per-turn context block. `PAI/PULSE/modules/telegram.ts` `buildPaiContextBlock()` mtime-caches the four constitutional files (DA_IDENTITY, PRINCIPAL_IDENTITY, PRINCIPAL_TELOS, PROJECTS) plus the two hot-layer memory files (`PRINCIPAL_MEMORY.md`, `DA_MEMORY.md`) for 60 seconds, re-reading whenever any per-file mtime changes. The 60s window is short enough to pick up an autonomic memory reviewer write within the same conversational burst, long enough to avoid re-reading on every prompt.

This is structurally distinct from the freshness cache documented above:
- **This file's cache** (`PAI/USER/CACHE/freshness.json`) backs the statusline FRESH line and `/api/freshness/summary`. Driven by `bump*` events + Pulse `invalidate()` + SessionStart hook. Schema = full A-F grade output.
...
- Library: `PAI/TOOLS/TelosFreshness.ts`
- Statusline cache writer: `PAI/TOOLS/FreshnessCache.ts`
- Statusline reader: `PAI/PAI_StatusLine.sh` (FRESH section)
- Cache file (private): `PAI/USER/CACHE/freshness.json`
- Migration: `PAI/TOOLS/MigrateContextFreshness.ts`
- Audit: `PAI/TOOLS/ContextAudit.ts`
- TELOS-specific docs: `PAI/USER/TELOS/README.md`
- Pulse module: `PAI/PULSE/modules/telos.ts`
- Interview workflow: `skills/Interview/Workflows/ContextCheckin.md`
- Generator (TELOS summary): `PAI/TOOLS/GenerateTelosSummary.ts`
- Generator (architecture summary): `PAI/TOOLS/ArchitectureSummaryGenerator.ts`
- ISA: `PAI/MEMORY/WORK/20260504-statusline-freshness-cache/ISA.md`
