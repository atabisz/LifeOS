...
> **LifeOS moves people from current state to ideal state** by writing down what done looks like as testable claims, then refining the writing until every claim survives every test it can be subjected to. The ISA is that written record — the LifeOS loop (`PAI/DOCUMENTATION/LifeOs/LifeOsThesis.md`) at single-artifact scale.
...
capabilities_invoked:           # Array of capabilities actually invoked via tool call
  - ISA                         # (closed enumeration, see PAI/ALGORITHM/capabilities.md)
  - SystemsThinking
  - FirstPrinciples
...
**Pulse surfacing** (planned with React component build — see `PAI/DOCUMENTATION/Pulse/PulseMetadata.md`):
...
- **Reference file pattern**: This spec lives at `~/.claude/PAI/DOCUMENTATION/ISA/IsaFormat.md`, not inline in CLAUDE.md. Saves ~2,500 tokens/response.
