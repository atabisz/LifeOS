...
| **Manual** | direct CLI / `/render-isa` slash command | The operator invokes `bun ~/.claude/PAI/TOOLS/ISARender.ts <slug-or-path>` from any shell. Bypasses the completion gate. Always works. |
...
`~/.claude/PAI/TOOLS/ISARender.ts` — single Bun script, ~470 LOC, hand-rolled ISA-aware markdown parser. No external dependencies; no `marked`, no Handlebars, no Bun install side effects.

| Surface | Behavior |
|---|---|
| Invocation | `bun ~/.claude/PAI/TOOLS/ISARender.ts <slug-or-path>` |
...
| File | Purpose |
|---|---|
| `PAI/TOOLS/ISARender/template.html` | Single HTML scaffold with named placeholders (`{{TITLE}}`, `{{CSS}}`, `{{HERO_CALLOUT}}`, `{{PHASE_BAR}}`, `{{SECTIONS}}`, etc.). Interpolation via plain `String.replaceAll`. |
| `PAI/TOOLS/ISARender/template.css` | Branded stylesheet inlined into the rendered HTML. Extracted from `_ULHORMOZI/_CONSULTINGTEMPLATES/test-mckinsey-refined-v2.html` with cover-page and network-visual selectors stripped, ISA-specific selectors added. |
...
- `PAI/DOCUMENTATION/ISA/IsaSystem.md` — twelve-section body, five identities, three-guardrail taxonomy
- `PAI/DOCUMENTATION/ISA/IsaFormat.md` — file-shape contract (frontmatter spec, locked section order)
- `PAI/ALGORITHM/v6.19.0.md` — the algorithm that produces ISAs, phase progression source
- `PAI/MEMORY/WORK/20260512-isa-html-mirror-system/ISA.md` — the ISA that designed this system
