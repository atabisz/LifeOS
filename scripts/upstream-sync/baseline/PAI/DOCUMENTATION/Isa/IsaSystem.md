...
- **Format spec** — `PAI/DOCUMENTATION/ISA/IsaFormat.md` — the file-shape contract: frontmatter fields, body section schemas, ID-stability rule, status markers.
...
- **Algorithm doctrine** — `PAI/ALGORITHM/v6.19.0.md` (or LATEST) — the invocation cadence: when each workflow fires across the seven Algorithm phases.
...
- **Algorithm** (`PAI/DOCUMENTATION/Algorithm/AlgorithmSystem.md`, `PAI/ALGORITHM/v6.19.0.md`) — invokes the ISA skill. OBSERVE invokes Scaffold; OBSERVE end + VERIFY invoke CheckCompleteness; PLAN may invoke ephemeral-extract; LEARN invokes Reconcile and Append. The Algorithm's seven phases are operational; the ISA is the durable artifact those phases write to.
...
- **Format spec (file-shape contract):** `PAI/DOCUMENTATION/ISA/IsaFormat.md`
...
- **Algorithm doctrine:** `PAI/ALGORITHM/v6.19.0.md` (current); `PAI/ALGORITHM/LATEST` for version pointer
