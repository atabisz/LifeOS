...
- `~/.claude/` — public LifeOS SYSTEM tree (what ships on GitHub). Contains the symlink `PAI/USER → ~/.config/PAI/USER`.
- `~/.config/PAI/USER/` — private user-data git working tree (the user's own private USER-data repo). Mounted via symlink so Claude Code's `@`-import resolver reaches identity/TELOS/config files at session start.
- `~/.config/PAI/USER/MEMORY/` — durable subset of memory (KNOWLEDGE, WORK/<slug>/ISA.md, RELATIONSHIP, WISDOM, PLANS, RESEARCH, STATE/work.json, BOOKMARKS, REFERENCE, SKILLS, PROJECT, TEAMS, PAISYSTEMUPDATES, VERIFICATION) git-tracked in the user's private USER-data repo; ephemeral subset (OBSERVABILITY, STATE caches, LEARNING signals, SECURITY artifacts, VOICE event log, _BROWSER_STATE) gitignored locally.

**Four allowed access patterns** (any fifth pattern is a boundary violation):
1. `PaiConfig.load()` typed loader (`PAI/TOOLS/PaiConfig.ts`) — primary channel for identity, voice, integrations, paths
2. Paths computed from `PaiConfig.paths.userDir + relative` — never from literal `PAI/USER/` strings in system code
...
**Two-repo sync** — `~/.claude/.git/hooks/pre-push` auto-commits and pushes `~/.config/PAI/USER/` to the user's private USER-data repo before each push from `~/.claude/`. The two repos stay in sync structurally. A private "kai update" / "push both repos" workflow wraps this with 4 boundary gates (USER-zone leak check, DenyListCheck, both-remotes-private confirmation, post-push HEAD verification).

Configuration files (`settings.json`, `CLAUDE.md`, `PAI_SYSTEM_PROMPT.md`) are directly edited; `settings.json` is split into `settings.system.json` (public) + `settings.user.json` (USER) merged at SessionStart by `PAI/TOOLS/MergeSettings.ts`. Identity values live in `settings.json` under `daidentity` and `principal` keys; private skills read credentials via `PaiConfig.ts` from `PAI/USER/CONFIG/PAI_CONFIG.toml`. Full contract: `PAI/DOCUMENTATION/SystemUserBoundary.md`.
...
| `PAI/TOOLS/lifeos.ts` | Launcher -- wires `--append-system-prompt-file` |
...
- **Version:** v6.18.0
- **Location:** `PAI/ALGORITHM/` (canonical pointer: `LATEST` → `v6.18.0.md`)
...
Turns a raw prompt into a posture: mode (MINIMAL/NATIVE/ALGORITHM), effort tier (E1–E5), the stated goal, the model rung, and which agent/vendor runs the work. Four stages — classify (`hooks/EffortRouter.hook.ts`), route the effort (Algorithm tier→level policy, which produces a *level*), select the model (`PAI/TOOLS/models.ts` four-level `EFFORT_MODEL` abstraction maps that level to a model), and dispatch the agent (dispatch-time `model` param). The Algorithm is the Router's biggest consumer, not part of it: the Router decides how to handle a prompt; the Algorithm/NATIVE executes within that decision. It exists to preserve dynamic range (fast on trivial work, deep on hard work) and to work around a harness limit — the main loop cannot switch its own model mid-turn, so the Router classifies up front and programs the model + effort of the agents the work dispatches.

- **Status:** Active (named + documented 2026-07-01)
- **Location:** `hooks/EffortRouter.hook.ts`, `PAI/TOOLS/models.ts`, `PAI/TOOLS/Inference.ts`, Algorithm Mode Classification doctrine
- **Full doc:** `PAI/DOCUMENTATION/Router/RouterSystem.md`
...
- **Location:** `PAI/MEMORY/` + `projects/*/memory/` + hot-layer at `PAI/USER/PRINCIPAL/PRINCIPAL_MEMORY.md` and `PAI/USER/DIGITAL_ASSISTANT/DA_MEMORY.md`
...
- **Full doc:** `PAI/DOCUMENTATION/Security/README.md`
...
- **Status:** Active
- **Source code:** `PAI/USER/CUSTOMIZATIONS/ARBOL/` (Cloudflare Workers repo)
...
| Document | Purpose |
|----------|---------|
| `PAI/DOCUMENTATION/LifeOs/LifeOsThesis.md` | **Canonical LifeOS thesis** -- what LifeOS is for, the core loop, LifeOS-MM, RIoT lineage, respark |
...
| `PAI/DOCUMENTATION/ISA/IsaFormat.md` | ISA format specification v2.0 -- the single source of truth for every Algorithm run |
| `PAI/DOCUMENTATION/Tools/Tools.md` | CLI utilities reference: Inference.ts (low/medium/high/max), ActivityParser, and others |
| `PAI/DOCUMENTATION/Observability/ObservabilitySystem.md` | Full Pulse API reference (~40 endpoints) under "API Reference" section |
| `PAI/DOCUMENTATION/Work/WorkSystem.md` | Work System architecture — four capture surfaces → private GitHub repo → Pulse + TASKLIST + agent claim, single config under `USER/WORK/` |
| `PAI/DOCUMENTATION/Router/RouterSystem.md` | The Router — prompt→posture decision layer (classify → route-effort → select-model → dispatch); four-level `EFFORT_MODEL` abstraction, classifier contract, tier→level policy, cross-vendor egress ceilings |
...
- **Decided:** 2026-06-10
- **Decision:** All Claude-family model routing speaks four effort LEVELS — `max` / `high` / `medium` / `low` — bound to concrete models in exactly one place: `EFFORT_MODEL` in `PAI/TOOLS/models.ts` (today max→Opus, high→Opus, medium→Sonnet, low→Haiku — the designed max→Fable flip is held pending the fable/opus economics decision). Doctrine, hooks, skills, and agents state intent (the level); the mapping resolves the model. v6.18.0 adds a per-task `intelligence` level on `## Features` the dispatch reads (down-route-only, fail-expensive, E1–E5).
...
- **Replaced:** An unnamed, undocumented routing behavior implied across four files. (AD-4 is left intact per the append-only convention; the max→Fable flip shipping 2026-07-01 supersedes AD-4's held-flip note — see `PAI/TOOLS/models.ts` `EFFORT_MODEL` for the current lineup.)
