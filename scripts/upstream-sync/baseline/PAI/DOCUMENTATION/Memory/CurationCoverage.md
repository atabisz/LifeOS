...
Defined in `PAI/TOOLS/MutationTier.ts` as a closed allowlist (default-deny):
...
|---|---|---|---|
| `USER/TELOS/PRINCIPAL_TELOS.md` | `PAI/TOOLS/GenerateTelosSummary.ts` | TELOS source-file change | Manual run; planned cron in P2 |
| `USER/TELOS/PAI_STATE.json` | `PAI/TOOLS/ComputeGap.ts` | Cron / Stop hook | Hourly |
| `DOCUMENTATION/ARCHITECTURE_SUMMARY.md` | `PAI/TOOLS/ArchitectureSummaryGenerator.ts` | `DocIntegrity.hook.ts` on Stop | Per session end |
...
| `MEMORY/STATE/work.json` | `hooks/ISASync.hook.ts`, `PAI/TOOLS/AlgoPhase.ts` | Phase transitions | Per change |
| `MEMORY/STATE/session-names.json` | `PAI/TOOLS/SessionRename.ts` (manual), `SessionAnalysis` (auto-name) | First prompt at session start; manual rename | One-shot per session |
...
| All of `PAI/DOCUMENTATION/` source files (except auto-generated summary) | Architecture docs; principal-curated (the ArchitectureSummary derives FROM these) |
| `CLAUDE.md`, `PAI_SYSTEM_PROMPT.md` | Constitutional layer; principal-curated |
...
1. Add the absolute path to the appropriate Tier set in `PAI/TOOLS/MutationTier.ts` (Tier A / B / C).
2. If Tier C — add an entry to `PROPOSAL_KIND_TO_FILES` in `PAI/TOOLS/MemoryTypes.ts` with a new `target_kind` value.
3. Extend the reviewer prompt in `PAI/TOOLS/MemoryReviewer.ts` with subtype guidance describing when to emit a proposal targeting the new kind.
