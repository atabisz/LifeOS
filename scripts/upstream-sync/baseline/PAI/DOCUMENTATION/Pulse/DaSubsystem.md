...
A single `DA_IDENTITY.md` per DA under `PAI/USER/DA/` carries everything: YAML frontmatter at the top holds the structured schema (core, voice, traits, autonomy, growth, anchors, etc.); the markdown body below holds the human-readable narrative (personality, writing style, relationship). One file, one source of truth, one place to edit. The `@`-import in `CLAUDE.md` loads the file unchanged; tooling that needs structured access parses the frontmatter slice.
...
|---|---|---|
| `PAI/USER/DIGITAL_ASSISTANT/DA_IDENTITY.md` (legacy flat md) + legacy `DA_IDENTITY.yaml` schema companion | `PAI/USER/DA/your-da/DA_IDENTITY.md` (single file with frontmatter) | Move structured fields into the file's leading YAML frontmatter; keep prose as the body. Delete the `.yaml` companion. |
| `PAI_CONFIG.yaml` DA section | `PAI/USER/DA/your-da/DA_IDENTITY.md` frontmatter `voice` / `personality` keys | Move voice config into frontmatter; keep PAI_CONFIG reference as pointer |
...
When someone installs LifeOS fresh, they need a DA identity. Currently this requires manually writing DA_IDENTITY.md and editing PAI_CONFIG.yaml. The interview system automates this.
...
// bun PAI/TOOLS/DAInterview.ts --update --da worker-da  # Update specific DA
```
...
```yaml
# PAI/USER/DA/worker-da/DA_IDENTITY.md frontmatter (simplified)
core:
  name: Echo
  full_name: Echo
...
```
Current State:
  PAI/USER/DIGITAL_ASSISTANT/DA_IDENTITY.md          (flat markdown)
