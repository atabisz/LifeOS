...
**Canonical thesis:** `PAI/DOCUMENTATION/LifeOs/LifeOsThesis.md` — the source of truth for what LifeOS is, what the DA is, and why Pulse exists.
...
- Voice ID is read from `settings.json` at module import, not hardcoded — so no `{{...}}` placeholder needs install-time resolution. `PAI_INSTALL/engine/actions.ts substituteTemplates` doesn't need to know about this module.
...
- Inference.ts: `PAI/TOOLS/Inference.ts` (CLI + library contract)
- VoiceServer: `PAI/PULSE/VoiceServer/voice.ts` (CLI `/notify` channel)
- ConversationStore: `PAI/PULSE/lib/conversation.ts` (`getHistory(): ConversationMessage[]` — timestamp surfaced for the idle filter)
- ShadowRelease substitution map: `skills/_LIFEOS/Tools/ShadowRelease.ts:482` (legacy `<principal-voice-id> → {{PAI_MAIN_VOICE_ID}}` rule still ships as defense-in-depth, but the live module no longer relies on it)
...
│   ├── user-index.ts         # LifeOS biography indexer (PAI/USER tree)
...
| LifeOS Index | `/api/user-index[?filter=stats\|publish\|stale\|gaps]` | Typed JSON of USER/ tree produced by `modules/user-index.ts` — spec: `PAI/DOCUMENTATION/LifeOs/LifeOsSchema.md` |
...
- **Disable:** `PAI_NO_SSE=1` env var → 503 on the endpoint, dashboard falls back to legacy 2s polling identically to pre-change behavior.
...
- **Atomic emitter:** `bun ~/.claude/PAI/TOOLS/AlgoPhase.ts <phase>` writes the current session's phase into `work.json` in ~22ms (p95 27ms). The Algorithm doctrine prescribes invocation at every phase transition. EffortRouter pre-emit (`markAlgorithmStarting`) eliminates the wrong-phase window at session start.
