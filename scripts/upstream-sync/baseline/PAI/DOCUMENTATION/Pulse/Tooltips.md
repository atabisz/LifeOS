...
- `PAI/PULSE/Observability/src/components/ui/chart.tsx` — the shared `ChartTooltip` used across every chart.
- `PAI/PULSE/Observability/src/components/FreshnessIndicator.tsx` — the per-panel freshness marker.
- `PAI/PULSE/Observability/src/app/globals.css` — tooltip styling.
- `PAI/PULSE/modules/tab-freshness.ts` — the freshness data each indicator reads.
...
- Pulse — the dashboard these tooltips live in: `PAI/DOCUMENTATION/Pulse/PulseSystem.md`
