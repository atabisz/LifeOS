...
- **Source of truth:** `PAI/USER/CUSTOMIZATIONS/Spinner/` — `verbs.json` (the working words), `icons.json`, `colors.json`, `anims.json`, plus a `README.md`.
- **Live config:** `settings.json` → `spinnerVerbs.verbs`.
- **Sync tool:** `PAI/TOOLS/SyncSpinnerAssets.ts` pushes verbs/icons/colors/anims from the customization dir out to the live config and to downstream projects.
- **Statusline renderer:** `PAI/PAI_StatusLine.sh`.
...
- Pulse — the Life Dashboard: `PAI/DOCUMENTATION/Pulse/PulseSystem.md`
