...
> Testing is how the Life OS knows what it knows (`PAI/DOCUMENTATION/LifeOs/LifeOsThesis.md`): the hill-climb only counts when each step is verified, and `bun test` is the canonical probe behind every ISC claim. An unverified gap-closure is a guess wearing a checkmark.
...
| `PAI/PULSE/modules/wiki.ts` | `test/pulse/modules/wiki.test.ts` |
| `PAI/TOOLS/Inference.ts` | `test/tools/Inference.test.ts` |
...
await using proc = Bun.spawn({
  cmd: [paiBunExe(), "PAI/TOOLS/Inference.ts", "--mode", "advisor"],
...
pathIgnorePatterns = [
  "**/node_modules/**",
  "**/PAI/PAI_RELEASES/**",  # shadow releases — no tests
  "**/PAI/MEMORY/**",         # ephemeral session data
  "**/.git/**",
]
coverage = false
...
4. **Phase 4.** Add the validation-pass layers: a `pai-doctrine.test.ts` that asserts `CLAUDE.md` and `PAI_SYSTEM_PROMPT.md` cross-references resolve; a `pai-secrets.test.ts` that asserts no API keys leak into the public release staging dir.
...
- ❌ Reading real `~/.claude/PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md` from a test. Use a fixture.
...
- Algorithm doctrine on ISA-as-test-harness: `~/.claude/PAI/ALGORITHM/v6.19.0.md` § Doctrine (line 17)
- ISA format spec: `~/.claude/PAI/DOCUMENTATION/ISA/IsaFormat.md`
