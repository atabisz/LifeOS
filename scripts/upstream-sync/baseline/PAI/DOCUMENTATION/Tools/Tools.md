...
> CLI-first is how the Life OS stays deterministic (`PAI/DOCUMENTATION/LifeOs/LifeOsThesis.md`): the hill-climb's moves are code you can script, test, and trust — prompts orchestrate, code executes.
...
bun ~/.claude/PAI/TOOLS/Inference.ts --level low "System prompt" "User prompt"

# Medium (Sonnet) - balanced reasoning, typical analysis
bun ~/.claude/PAI/TOOLS/Inference.ts --level medium "System prompt" "User prompt"

# High (Opus) - deep reasoning, strategic decisions
bun ~/.claude/PAI/TOOLS/Inference.ts --level high "System prompt" "User prompt"

# Max (Fable) - hardest reasoning, top tier
bun ~/.claude/PAI/TOOLS/Inference.ts --level max "System prompt" "User prompt"

# With JSON output
bun ~/.claude/PAI/TOOLS/Inference.ts --json --level low "Return JSON" "Input"

# Custom timeout
bun ~/.claude/PAI/TOOLS/Inference.ts --level medium --timeout 60000 "Prompt" "Input"
```
...
**Location:** `~/.claude/PAI/TOOLS/models.ts` (registry) + `~/.claude/PAI/TOOLS/UpdateModels.ts` (updater).
...
- **Architecture**: `~/.claude/PAI/DOCUMENTATION/LifeosSystemArchitecture.md` (master architecture reference)
