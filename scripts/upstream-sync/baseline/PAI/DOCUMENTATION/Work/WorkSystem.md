...
> The Work System is the hill-climb's ledger (`PAI/DOCUMENTATION/LifeOs/LifeOsThesis.md`). Every captured unit of work is a step taken toward ideal state, and the TELOS sweep is the loop closing on itself: an active goal with no open issue is a declared ideal state with no next move — exactly the gap the OS exists to surface.
...
### 3. Periodic sweep — `PAI/TOOLS/WorkSweep.ts` + launchd 60min

Runs every 60 minutes via `~/Library/LaunchAgents/com.lifeos.worksweep.plist` (installed via `bun ~/.claude/PAI/TOOLS/InstallWorkSweep.ts`). Four sub-sweeps:
...
### Pulse Work tab — `PAI/PULSE/modules/work.ts`
...
|-------|----------|----------|-------------------|
| **System code** | `~/.claude/hooks/`, `~/.claude/PAI/PULSE/`, `~/.claude/PAI/TOOLS/`, `~/.claude/skills/_ULWORK/` | Hooks, modules, CLIs, skill workflows | YES — scrubbed, public-clean |
| **User config** | `~/.claude/PAI/USER/WORK/` | `labels.yml`, `config.yaml`, `work_repo.json`, `README.md` | NO — USER zone, excluded by containment |
...
4. Restart Pulse: `bash ~/.claude/PAI/PULSE/manage.sh restart`
5. `bun ~/.claude/PAI/TOOLS/InstallWorkSweep.ts` to register the launchd job
...
| `PAI/TOOLS/WorkSweep.ts` | Periodic sweep — four sub-sweeps + TASKLIST regen |
| `PAI/TOOLS/com.lifeos.worksweep.plist.template` | launchd plist template |
| `PAI/TOOLS/InstallWorkSweep.ts` | Materializes template, bootstraps launchd job |
| `PAI/PULSE/modules/work.ts` | Pulse kanban renderer with source badges |
...
- ISA spec: `PAI/DOCUMENTATION/ISA/IsaFormat.md`
- System/user boundary doctrine: `PAI/DOCUMENTATION/SystemUserBoundary.md`
...
- Redesign ISA (provenance): `PAI/MEMORY/WORK/20260525-213259_ulwork-sync-redesign/ISA.md`
