...
export async function runCLI(): Promise<void> {
  ...
  try {
    ...
    print(`  ${c.gray}Each subdirectory under ~/.config/PAI/USER/ has a README.md explaining what goes inside and how to customize it.${c.reset}`);
    print(`  ${c.gray}Start with:${c.reset}`);
    print(`     ${c.bold}~/.config/PAI/USER/README.md${c.reset}                       ${c.gray}— full layout map${c.reset}`);
    print(`     ${c.bold}~/.config/PAI/USER/TELOS/README.md${c.reset}                 ${c.gray}— missions, goals, problems, strategies${c.reset}`);
    print(`     ${c.bold}~/.config/PAI/USER/DIGITAL_ASSISTANT/README.md${c.reset}     ${c.gray}— your DA's identity, voice, personality${c.reset}`);
    print(`     ${c.bold}~/.config/PAI/USER/PRINCIPAL/README.md${c.reset}             ${c.gray}— your identity, resume, writing style${c.reset}`);
    print(`     ${c.bold}~/.config/PAI/USER/PROJECTS.md${c.reset}                     ${c.gray}— project registry + routing aliases${c.reset}`);
    print(`     ${c.bold}~/.config/PAI/USER/SECURITY/README.md${c.reset}              ${c.gray}— bash/path rules (already has working defaults)${c.reset}`);
    print(`     ${c.bold}~/.config/PAI/USER/CONFIG/README.md${c.reset}                ${c.gray}— credentials and LifeOS config${c.reset}`);
    print("");
    print(`  ${c.lightBlue}${c.bold}Where your data actually lives:${c.reset}`);
    print(`  ${c.gray}System code lives at ${c.bold}~/.claude/${c.reset}${c.gray}. Your private user data lives at ${c.bold}~/.config/PAI/USER/${c.reset}${c.gray}.${c.reset}`);
    print(`  ${c.gray}${c.bold}~/.claude/PAI/USER${c.reset}${c.gray} is a symlink to ${c.bold}~/.config/PAI/USER${c.reset}${c.gray} — this separation is required for LifeOS to work.${c.reset}`);
    print(`  ${c.gray}When you edit ${c.bold}~/.claude/PAI/USER/...${c.reset}${c.gray} you're really editing ${c.bold}~/.config/PAI/USER/...${c.reset}${c.gray} (same file).${c.reset}`);
    print(`  ${c.gray}${c.bold}Optional but recommended:${c.reset}${c.gray} turn ${c.bold}~/.config/PAI/${c.reset}${c.gray} into a private git repo so you have versioned history of everything you write:${c.reset}`);
    ...
    print(`  ${c.gray}•${c.reset} Anything you write under ${c.bold}PAI/USER/${c.reset}${c.gray} stays on your machine — it never ships in any LifeOS release.${c.reset}`);
    print("");
