...
const BACKUP_MIGRATION_CANDIDATES = [
  ["PAI", "USER", "DIGITAL_ASSISTANT", "DA_IDENTITY.md"].join("/"),
  ["PAI", "USER", "PRINCIPAL_IDENTITY.md"].join("/"),
  ["PAI", "USER", "PRINCIPAL", "PRINCIPAL_IDENTITY.md"].join("/"),
  ["PAI", "USER", "PROJECTS.md"].join("/"),
  ["PAI", "USER", "TELOS", "PRINCIPAL_TELOS.md"].join("/"),
  ["PAI", "USER", "TELOS", "TELOS.md"].join("/"),
  ["PAI", "USER", "CONTACTS.md"].join("/"),
  ["PAI", "USER", "DEFINITIONS.md"].join("/"),
  ["PAI", "USER", "CANONICAL_CONTENT.md"].join("/"),
  ["PAI", "USER", "PRINCIPAL", "RESUME.md"].join("/"),
  ["PAI", "USER", "PRINCIPAL", "WRITINGSTYLE.md"].join("/"),
  ["PAI", "USER", "PRINCIPAL", "PRONUNCIATIONS.json"].join("/"),
] as const;
...
function appendMigrationManifest(entry: MigrationManifestEntry, paiDir: string): void {
  const manifestPath = join(paiDir, "PAI", "MEMORY", "STATE", "install-migration.json");
...
export function substituteTemplates(rootDir: string, vars: TemplateVars): SubstitutionResult {
  ...
  for (const filePath of [
    ...
    join(rootAbs, "PAI", "PAI_SYSTEM_PROMPT.md"),
  ]) {
  ...
  for (const scopedDir of [
    join(rootAbs, "PAI", "USER"),
    join(rootAbs, "PAI", "DOCUMENTATION"),
    join(rootAbs, "PAI", "ALGORITHM"),
    join(rootAbs, "agents"),
    join(rootAbs, "hooks"),
    join(rootAbs, "skills"),
  ]) {
...
// skills, PAI/USER, PAI/DOCUMENTATION, PAI/ALGORITHM). Empty values fall back
 ...
 *   1. `~/.config/PAI/.env` — LifeOS's canonical key store. Lives outside
...
// Relocate ~/.claude/PAI/USER → ~/.config/PAI/USER and replace the live-tree
// path with a symlink. Implements the post-Phase-G structural contract from
// PAI/DOCUMENTATION/SystemUserBoundary.md: system code lives at ~/.claude/,
// private user data lives at ~/.config/PAI/USER/, and the live-tree path is
...
async function relocateUserToConfigDir(
  ...
  const liveUserDir = join(paiDir, "PAI", "USER");
  ...
  if (existsSync(liveUserDir)) {
    try {
      if (lstatSync(liveUserDir).isSymbolicLink()) {
        ...
        await emit({
          event: "message",
          content: `User data already separated: ~/.claude/PAI/USER → ${target}. No relocation needed.`,
        });
  ...
  if (configUserHasContent) {
    if (existsSync(liveUserDir)) {
      ...
      try {
        ...
        await emit({
          event: "message",
          content: `Could not remove ~/.claude/PAI/USER before symlinking: ${err instanceof Error ? err.message : String(err)}. Manual fix: rm -rf ${liveUserDir} && ln -s ${configUserDir} ${liveUserDir}`,
        });
        return;
      }
    }
    try {
      ...
      await emit({
        event: "message",
        content: `Linked ~/.claude/PAI/USER → ${configUserDir}. Existing user content preserved.`,
      });
    } catch (err) {
      await emit({
        event: "message",
        content: `Could not create ~/.claude/PAI/USER symlink: ${err instanceof Error ? err.message : String(err)}. Manual fix: ln -s ${configUserDir} ${liveUserDir}`,
      });
      return;
    }
  } else {
    ...
    try {
      ...
      // been moved to configUserDir, and ~/.claude/PAI/USER is now absent.
    ...
    await emit({
      event: "message",
      content: `Set up user-data separation: USER content lives at ${configUserDir}, with ~/.claude/PAI/USER as a symlink to it.`,
    });
  }
...
async function emitUserSeparationNotice(
  ...
  await emit({
    event: "message",
    content: `IMPORTANT — system/data separation: system code lives at ~/.claude/; your private user data lives at ${configDir}/. The symlink ~/.claude/PAI/USER → ${configUserDir} is required for LifeOS to work. Don't delete or change it.`,
  });
  await emit({
    event: "message",
...
export async function moveExistingClaudeToBackup(
  ...
  // The installer is executing from ~/.claude/PAI/PAI_INSTALL right now.
  // Never remove ~/.claude wholesale here; instead clear only the parts the
  // fresh install will replace and explicitly preserve PAI/PAI_INSTALL.
...
export async function runRepository(
  ...
  // ~/.config/PAI/USER/, with ~/.claude/PAI/USER as a symlink. Runs before
  // migrateUserContext so the legacy-skill migration writes through the
  // symlink into the canonical location.
  const configDirForRepo = state.detection?.configDir || join(homedir(), ".config", "PAI");
  await emit({ event: "progress", step: "repository", percent: 80, detail: "Separating user data into ~/.config/PAI/USER..." });
...
export async function runConfiguration(
  ...
  try {
    ...
    // PAI/USER is a symlink to ~/.config/PAI/USER on v6.0+ installs;
    // countFiles follows the symlink and counts the canonical scaffold.
    // The legacy ~/.claude/skills/PAI/USER path doesn't exist on fresh
  ...
  const aliasLine = `alias pai='bun ${join(paiDir, "PAI", "TOOLS", "lifeos.ts")}'`;
...
async function installDeriver(paiDir: string, emit: EngineEventHandler): Promise<boolean> {
  const manageScript = join(paiDir, "PAI", "PULSE", "manage-deriver.sh");
  if (!existsSync(manageScript)) {
  ...
  try {
    ...
    await emit({ event: "message", content: "Deriver install did not complete. You can run it later: bash ~/.claude/PAI/PULSE/manage-deriver.sh install" });
    return false;
  } catch {
    return false;
  }
  ...
  if (voiceServerReady) {
    ...
    if (process.platform === "darwin") {
      ...
      const installDeriverChoice = await getChoice("install-deriver", "Install the proactive deriver loop?", [
        ...
        { label: "Skip — no proactive hypotheses", value: "skip", description: "/hypotheses tab will show empty state. Install later: bash ~/.claude/PAI/PULSE/manage-deriver.sh install" },
      ], daName);

      if (installDeriverChoice === "yes") {
        await installDeriver(paiDir, emit);
      } else {
        await emit({ event: "message", content: "Deriver skipped. Install later: bash ~/.claude/PAI/PULSE/manage-deriver.sh install" });
      }
    }
  }
