...
function collectValidationFiles(paiDir: string): string[] {
  ...
  const shouldSkip = (path: string): boolean =>
    ...
    path.includes("/.git/") ||
    path.includes("/PAI/MEMORY/") ||
    path.includes("/PAI/PAI_INSTALL/");
  ...
  addFile(join(paiDir, "PAI", "PAI_SYSTEM_PROMPT.md"));
  addRootFileExplicit(join(paiDir, "LICENSE"));
  addRootFileExplicit(join(paiDir, "bunfig.toml"));
  walk(join(paiDir, "PAI", "USER"));
  walk(join(paiDir, "PAI", "DOCUMENTATION"));
  walk(join(paiDir, "PAI", "ALGORITHM"));
...
// Verify the post-Phase-G system/user separation contract: ~/.claude/PAI/USER
// must be a symlink whose target is ~/.config/PAI/USER. A silent symlink
...
export function runSymlinkContractCheck(paiDir: string): ValidationCheck {
  ...
  const expectedTarget = join(homedir(), ".config", "PAI", "USER");
