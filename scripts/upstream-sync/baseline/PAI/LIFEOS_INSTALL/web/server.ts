...
// ~/.claude/PAI/PAI_INSTALL/, so PAI/VERSION sits two levels up. The
// {{PAI_VERSION}} placeholder in HTML responses is substituted on serve
...
const PAI_VERSION_FILE = join(import.meta.dir, "..", "..", "VERSION");
const PAI_VERSION: string = (() => {
  try { return readFileSync(PAI_VERSION_FILE, "utf8").trim() || "?.?.?"; }
  catch { return "?.?.?"; }
})();
function injectVersion(html: string): string {
  return html.split("{{PAI_VERSION}}").join(PAI_VERSION);
}
...
const server = Bun.serve({
  ...
  fetch(req, server) {
    ...
    if (existsSync(fullPath)) {
      ...
      // Substitute {{PAI_VERSION}} in HTML responses so the installer
