...
# PAI_VERSION: read from PAI/VERSION (canonical, also read by install.sh,
...
# hook-spawn contexts where HOME/PAI_DIR may not resolve.
PAI_VERSION=""
for _pai_v_path in \
    "$PAI_DIR/VERSION" \
    "$HOME/.claude/PAI/VERSION" \
    "/Users/$(id -un 2>/dev/null)/.claude/PAI/VERSION" \
    "$(eval echo ~"$(id -un 2>/dev/null)")/.claude/PAI/VERSION"; do
    if [ -n "$_pai_v_path" ] && [ -f "$_pai_v_path" ]; then
        PAI_VERSION="$(cat "$_pai_v_path" 2>/dev/null | tr -d '[:space:]')"
        [ -n "$PAI_VERSION" ] && break
    fi
done
...
# typically a symlink to $HOME/.config/PAI/.env). The historical $HOME/.claude/PAI/.env
...
printf '%s' "$input" > "$PAI_DIR/MEMORY/STATE/statusline-stdin-debug.json" 2>/dev/null
...
if [ "$MODE" != "nano" ]; then
{
    ...
    # full `bun GetCounts.ts` walked PAI/USER/, MEMORY/LEARNING/, MEMORY/WORK/,
    ...
    _work_dirs=("$PAI_DIR"/MEMORY/WORK/*/)
    ...
    _hooks_cnt=$(bun "$HOME/.claude/PAI/TOOLS/GetCounts.ts" --single hooks 2>/dev/null || echo 0)
...
# curated 1024-line corpus at $PAI_DIR/USER/PRINCIPAL/Quotes.txt. Selection is a
...
# NOTE: DA_NAME, PAI_VERSION, input JSON, harness_name, harness_version, cc_version, model_name, dir_name
...
if [ "$MODE" != "normal" ]; then
    ...
    case "$MODE" in
        nano)
            ...
            printf "${PAI_A}${PAI_LOGO}${RESET}  ${PAI_P}LI${PAI_A}FE${PAI_I}OS${RESET} ${CTX_PRIMARY}◉${RESET}${_pct_color}${_raw_pct}%%${RESET}
"
            # Line 2: git + learning
...
"
        ...
        mini)
            ...
            printf "${SLATE_600}──${RESET} ${PAI_A}${PAI_LOGO}${RESET}  ${PAI_P}LI${PAI_A}FE${PAI_I}OS${RESET} ${SLATE_600}──${RESET} ${PAI_CITY}${location_city}${RESET} ${SLATE_600}│${RESET} ${PAI_TIME}${current_time}${RESET} ${SLATE_600}│${RESET} ${PAI_WEATHER}${weather_str}${RESET}
"
...
if [ -n "$session_display" ]; then
    printf "${PAI_P}LI${PAI_A}FE${PAI_I}OS${RESET} ${SLATE_600}│${RESET} ${_hdr_loc}  ${PAI_TIME}${current_time}${RESET}  ${PAI_WEATHER}${weather_str}${RESET} ${SLATE_600}│${RESET} ${PAI_SESSION}${session_display}${RESET}\n"
else
    ...
    printf "${PAI_P}LI${PAI_A}FE${PAI_I}OS${RESET} ${SLATE_600}│${RESET} ${_hdr_loc}  ${PAI_TIME}${current_time}${RESET}  ${PAI_WEATHER}${weather_str}${RESET} ${SLATE_600}${_hdr_dashes}${RESET}\n"
fi
printf "${SLATE_600}%s${RESET}\n" "$SEP_DASHED"
...
_pm_work_json="$PAI_DIR/MEMORY/STATE/work.json"
_pm_router_log="$PAI_DIR/MEMORY/OBSERVABILITY/effort-router.jsonl"
...
_pm_level_model=""
_pm_models_ts="$PAI_DIR/TOOLS/models.ts"
...
_review_state_file="$PAI_DIR/MEMORY/OBSERVABILITY/review-state.json"
_memory_health_file="$PAI_DIR/MEMORY/OBSERVABILITY/memory-health.jsonl"
_memory_config_file="$PAI_DIR/USER/CONFIG/memory-review.json"
_reviewer_runs_file="$PAI_DIR/MEMORY/OBSERVABILITY/reviewer-runs.jsonl"
_principal_memory_file="$PAI_DIR/USER/PRINCIPAL/PRINCIPAL_MEMORY.md"
_da_memory_file="$PAI_DIR/USER/DIGITAL_ASSISTANT/DA_MEMORY.md"
...
printf "${SLATE_400}HARN:${RESET} ${PAI_A}${_har_display}${RESET} ${SLATE_600}│${RESET} ${SLATE_400}MODEL:${RESET} ${PAI_A}${_model_display}${RESET} ${SLATE_600}│${RESET} ${SLATE_400}LIFEOS:${RESET} ${PAI_A}${PAI_VERSION}${RESET} ${SLATE_600}│${RESET} ${SLATE_400}ALGO:${RESET} ${PAI_A}${ALGO_VERSION}${RESET}\n"
...
_AGENT_STARTS="$PAI_DIR/MEMORY/OBSERVABILITY/agent-starts.json"
...
_collect_ctx_file "$PAI_DIR/PAI_SYSTEM_PROMPT.md" "PAI_SYSTEM_PROMPT.md"
