...
//
// Disable via env var: PAI_NO_SSE=1 (returns 503).
//
...
const SSE_DISABLED = process.env.PAI_NO_SSE === "1"
...
function handleAlgorithmStreamApi(req: Request): Response {
  if (SSE_DISABLED) {
    return new Response("SSE disabled via PAI_NO_SSE=1", { status: 503 })
  }
...
// 1. Constitutional Security Protocol in PAI_SYSTEM_PROMPT.md
// 2. Native Claude Code permissions.deny in settings.json
// 3. One ~50-LOC PromptInjection.hook.ts on WebFetch/WebSearch
// (See PAI/DOCUMENTATION/Security/README.md for the full model.)
//
...
function handleSecurityApi(): Response {
  ...
  return Response.json({
    model: "minimal-v1",
    description:
      "Three-layer defense: constitutional rule (system prompt), native permissions.deny (settings.json), one PromptInjection hook (WebFetch/WebSearch). See PAI/DOCUMENTATION/Security/README.md.",
    denyList,
    hooks,
  })
}
...
async function handleSecurityRulesMutation(_req: Request): Promise<Response> {
  return Response.json(
    { error: "SECURITY_RULES.md removed in security simplification. The model is the security boundary; see PAI/DOCUMENTATION/Security/README.md." },
    { status: 410 },
  )
}
...
// Memory Graph — serves MEMORY/GRAPH/graph.json (built by PAI/TOOLS/MemoryGraph.ts)
...
function handleMemoryGraphApi(): Response {
  try {
    ...
    if (!existsSync(graphPath)) {
      return Response.json({ nodes: [], edges: [], communities: [], built: null, note: "run: bun PAI/TOOLS/MemoryGraph.ts build --all" })
    }
...
// freedom percentages from PAI_STATE.json. Missing source dimension contributes 0.
function buildDimensionsFromIdealState(): Array<{ id: string; label: string; cur: number; ideal: number; velo: number; color: string }> {
  ...
  const statePath = join(TELOS_DIR, "PAI_STATE.json")
  ...
  if (existsSync(statePath)) {
    try {
      ...
      // PAI_STATE.json corrupt — fall through with all zeroes
    }
  }
...
async function handleTelosOverview(): Promise<Response> {
  try {
    // Single source of truth: PAI/USER/TELOS/TELOS.md, split by H2 sections.
...
export async function handleObservabilityRequest(req: Request): Promise<Response | null> {
  ...
  if (method === "GET") {
    ...
    if (pathname === "<REDACTED:high-entropy>") return handleToolFailuresApi()
