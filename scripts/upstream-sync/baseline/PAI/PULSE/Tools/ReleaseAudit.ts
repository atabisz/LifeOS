...
function loadProhibitedStrings(): string[] {
  const candidates = [
    process.env.PAI_RELEASE_AUDIT_STRINGS,
    join(homedir(), ".config/PAI/USER/CONFIG/release-audit-strings.json"),
    join(homedir(), ".claude/PAI/USER/CONFIG/release-audit-strings.json"),
...
for (const file of files) {
  ...
  if (rel.startsWith("PAI/MEMORY/PULSE_DATA/") || rel.includes("/MEMORY/PULSE_DATA/")) {
    issues.push({ path: rel, rule: "containment", detail: "PULSE_DATA/ contents must not ship" });
    continue;
  }

  if (rel.startsWith("PAI/USER/") && (rel.endsWith(".md") || rel.endsWith(".markdown"))) {
    if (rel.startsWith("PAI/USER/_TEMPLATES/")) continue;
    try {
