...
const PAI_ROOT = resolve((import.meta as unknown as { dir: string }).dir, "..", "..", "..");
...
function checkPromptFileExists(promptFile: string): boolean {
  return existsSync(resolve(PAI_ROOT, promptFile));
}
  ...
  for (const file of files) {
    ...
    if (!checkPromptFileExists(manifest.adapterPromptFile)) {
      issues.push({
        ...
        message: `prompt file not found: ${manifest.adapterPromptFile} (resolved against ${PAI_ROOT})`,
      });
    }
  }
