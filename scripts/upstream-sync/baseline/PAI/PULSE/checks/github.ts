...
const LEGACY_STATE_FILE = join(HOME, ".claude", "PAI", "PULSE", "state", "github-seen.json")
const STATE_FILE = join(HOME, ".claude", "PAI", "PULSE", "state", "github-seen.jsonl")
...
async function main() {
  ...
  // Mark all current PRs as seen. PAI_PULSE_REPOS is expected to be
  // "owner/name" pairs, so `pr.repo` should always include "/". If it
  // doesn't, prefix with PAI_GITHUB_ORG (env-driven, no hardcoded org).
  const ghOrg = process.env.PAI_GITHUB_ORG ?? "YOUR-GITHUB-ORG"
