...
const USER_ROOT = resolve(HOME, ".claude", "PAI", "USER");
const EDITS_LOG = resolve(HOME, ".claude", "PAI", "MEMORY", "OBSERVABILITY", "pulse-edits.jsonl");
...
function isContainmentPath(absPath: string): boolean {
  const rel = absPath.replace(resolve(HOME, ".claude", "PAI") + "/", "");
