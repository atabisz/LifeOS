...
export interface Job {
   ...
   *
   * - "system": from PAI/PULSE/PULSE.toml (ships in public release)
   * - "user":   from PAI/USER/CONFIG/PULSE.user.toml (private, stripped at release)
   */
  _source?: JobSource
}
...
//
// USER_CRON_PATH points inside PAI/USER/**, which is already declared a
...
export const USER_CRON_PATH = join(
  process.env.HOME ?? "~",
  ".claude", "PAI", "USER", "CONFIG", "PULSE.user.toml",
)
...
// ── Config Loading ──
//
// Loads PULSE.toml (system) and PAI/USER/CONFIG/PULSE.user.toml (user)
...
export async function spawnClaude(prompt: string, opts: { model: string; timeoutMs?: number }): Promise<string> {
  ...
  // silently overrides OAuth. Mirrors PAI/TOOLS/Inference.ts:116-117.
