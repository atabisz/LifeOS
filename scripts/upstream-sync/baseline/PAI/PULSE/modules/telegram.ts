...
// SDK session billed to the API. Mirrors PAI/TOOLS/Inference.ts:116-117.
...
// PaiConfig — `[da.voices.main] voice_id` in PAI/USER/CONFIG/PAI_CONFIG.toml.
...
export async function buildPaiContextBlock(query?: string): Promise<string> {
  ...
  await Promise.all(probeRels.map(async (rel) => {
    try {
      const st = await stat(join(PAI_DIR, rel))
  ...
  const readSafe = async (rel: string, label: string): Promise<string> => {
    try {
      const raw = await readFile(join(PAI_DIR, rel), "utf8")
      return raw.trim()
    } catch (err) {
  ...
  const memoryBlocks = MEMORY_SOURCES.map(({ rel, title }) => {
    try {
      const r = readMemory(join(PAI_DIR, rel))
      if ("code" in r) {
...
// PAI/PULSE/lib/telegram-proposals.ts for unit testability. This module
  ...
  bot.on("message:text", async (ctx) => {
    ...
    try {
      ...
      const sdkOptions: Record<string, unknown> = {
        ...
        // DocCrossRefIntegrity all check PAI_NOTIFICATION_CHANNEL and skip
        ...
        env: { ...process.env, PAI_NOTIFICATION_CHANNEL: "telegram" },
...
A belt-and-suspenders egress sanitizer (PAI/PULSE/lib/strip-mode-scaffolding.ts) catches any of these markers and strips them before the message ships — but cleaner to never emit them in the first place. If the sanitizer fires on your output that's a leak telemetry signal.
...
- Do not invent project names, people, dates. If unsure, check the context block or the files directly.`,
      ...
      // Source: PAI/PULSE/lib/strip-mode-scaffolding.ts.
