...
const PULSE_STATE_DIR = join(PAI_DIR, "PULSE", "state");
...
function extractPrincipalGoal(slug: string | undefined): string | undefined {
  ...
  const isaPath = join(HOME, ".claude", "PAI", "MEMORY", "WORK", slug, "ISA.md");
...
function setupTemplate(reason: string): Response {
  ...
  const body = {
    ...
    instructions: [
      ...
      "Restart Pulse so this module re-reads work_repo.json: `bun ~/.claude/PAI/PULSE/manage.sh restart`.",
