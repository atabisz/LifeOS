...
const DEFAULT_WORK_DIR = path.join(PAI_DIR, "MEMORY", "WORK");
const DEFAULT_ARCH_DOC = path.join(PAI_DIR, "DOCUMENTATION", "LifeosSystemArchitecture.md");
...
function writeSelftestFixture(tempDir: string): { workDir: string; docPath: string } {
  ...
  const isaDir = path.join(workDir, "<REDACTED:high-entropy>");
  const incompleteIsaDir = path.join(workDir, "<REDACTED:high-entropy>");
...
function runSelftest(): string {
  ...
  try {
    ...
    assertSelftest(!selected.some((decision) => decision.slug === "<REDACTED:high-entropy>"), "h");
    ...
    assertSelftest(selectedWithIncomplete.some((decision) => decision.slug === "<REDACTED:high-entropy>"), "i");
