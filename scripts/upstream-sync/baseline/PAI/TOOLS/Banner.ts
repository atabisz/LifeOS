...
const PAI_LIGHT = rgb(147, 197, 253);  // pale blue (base + steps)
const PAI_NAVY = rgb(30, 58, 138);     // dark navy (mid blocks)
const PAI_BRIGHT = rgb(37, 99, 235);   // bright blue (top cap)

// LIFEOS wordmark — single source of truth for the header text, kept identical
// to the status line (PAI_StatusLine.sh: ${PAI_P}LI${PAI_A}FE${PAI_I}OS).
...
function lifeosLogoRows(cellW: number): string[] {
  ...
  const L = c(PAI_LIGHT), N = c(PAI_NAVY), Z = c(PAI_BRIGHT);
  return [
    `${e}${e}${e}${Z}`,
    `${e}${e}${e}${N}`,
    `${e}${e}${L}${L}`,
...
function getStats(): SystemStats {
  ...
  try {
    ...
    const daPath = join(CLAUDE_DIR, "PAI", "USER", "DIGITAL_ASSISTANT", "DA_IDENTITY.md");
    if (existsSync(daPath)) {
      ...
      if (m) {
        ...
        const core = fm.core ?? {};
        name = core.display_name || core.name || "PAI";
  ...
  try {
    // LifeOS version: ~/.claude/PAI/VERSION (single source of truth)
    const versionPath = join(CLAUDE_DIR, "PAI", "VERSION");
    if (existsSync(versionPath)) {
      paiVersion = readFileSync(versionPath, "utf-8").trim() || paiVersion;
    }
    // Algorithm version: ~/.claude/PAI/ALGORITHM/LATEST
  ...
  // recursed into PAI/USER/ (123k files) for keys nothing displays.
  let skills = 0, hooks = 0, sessions = 0;
  const getCountsPath = join(CLAUDE_DIR, "PAI", "TOOLS", "GetCounts.ts");
  try {
