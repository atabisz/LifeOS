 ...
 *
 * Local queue: ~/.claude/PAI/MEMORY/STATE/feed-candidates.db (bun:sqlite)
 *
 * USAGE:
...
const HOME = homedir();
const DB_PATH = join(HOME, ".claude/PAI/MEMORY/STATE/feed-candidates.db");
...
async function cmdHarvest() {
  ...
  console.log(`Shortlist: bun ${join(HOME, ".claude/PAI/TOOLS/BlogDiscovery.ts")} top --n 50`);
}
