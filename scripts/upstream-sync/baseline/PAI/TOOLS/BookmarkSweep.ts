 ...
 *
 *   bun ~/.claude/PAI/TOOLS/BookmarkSweep.ts                 # apply
 *   bun ~/.claude/PAI/TOOLS/BookmarkSweep.ts --dry-run       # classify only
 *   bun ~/.claude/PAI/TOOLS/BookmarkSweep.ts --max-create 3  # cap issues
 *
 * What it does:
...
const INFERENCE_TOOL = join(PAI_DIR, "TOOLS", "Inference.ts");
