...
const CLAUDE_DIR = dirname(PAI_DIR);
...
function scanSystemPromptModel(info: BodyInfo): Finding[] {
  // Single source of truth: PAI/TOOLS/models.ts CURRENT.opus. No hardcoded ID —
...
function normalizeReference(raw: string): string | null {
  ...
  if (value.startsWith("PAI/")) return join(CLAUDE_DIR, value);
  if (value.startsWith("~/.claude/PAI/")) return join(HOME, value.slice(2));
  if (value.startsWith(`${HOME}/.claude/PAI/`)) return value;
  return null;
}
