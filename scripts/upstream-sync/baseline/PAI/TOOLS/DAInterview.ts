 ...
 *   bun PAI/TOOLS/DAInterview.ts --update           # Update primary DA
 *
 * Operates ONLY on the primary DA (USER/DIGITAL_ASSISTANT/). Worker DAs in
 * USER/PAI_WORKERS/ are not touched by this tool.
 *
 * Creates:
 *   PAI/USER/DIGITAL_ASSISTANT/DA_IDENTITY.md     (single file: YAML frontmatter + prose body)
 *   PAI/USER/DIGITAL_ASSISTANT/growth.jsonl
 *   PAI/USER/DIGITAL_ASSISTANT/opinions.yaml
 *   PAI/USER/DIGITAL_ASSISTANT/diary.jsonl
 *
 * Reads roster scaffolding (presets) from:
 *   PAI/USER/PAI_WORKERS/_presets.yaml
 */
...
// PAI/TOOLS/DAInterview.ts. This tool is primary-DA-only; the primary lives at
// USER/DIGITAL_ASSISTANT/ (flat — files live at the dir root, not in a name
// subdir). Worker DAs in USER/PAI_WORKERS/ are out of scope. Roster scaffolding
// (presets/registry) lives at USER/PAI_WORKERS/.

const SCRIPT_DIR = dirname(fileURLToPath(import.meta.url));
const PAI_DIR = join(SCRIPT_DIR, "..");
const DA_DIR = join(PAI_DIR, "USER/DIGITAL_ASSISTANT");
const PRESETS_PATH = join(PAI_DIR, "USER/PAI_WORKERS/_presets.yaml");
const REGISTRY_PATH = join(PAI_DIR, "USER/PAI_WORKERS/_registry.yaml");
