...
- "<REDACTED:high-entropy>" (too generic)
- "error" (too short, not descriptive)
- "<REDACTED:high-entropy>" (still too generic)
...
Generate the 8-word description:`;
  ...
  return fallback || '<REDACTED:high-entropy>';
}
    ...
    try {
      ...
      if (entry.rating >= 1 && entry.rating <= 3) {
        ...
        if (entryDate >= oneMonthAgo) {
          ...
          const desc = (entry.sentiment_summary || '<REDACTED:high-entropy>')
            .toLowerCase()
            .replace(/[^a-z0-9\s]/g, '')
            .split(/\s+/)
            .slice(0, 8)
