 ...
 *
 * Usage:
 *   bun ~/.claude/PAI/TOOLS/Grok.ts "<query>"                 # web + X (default)
 *   bun ~/.claude/PAI/TOOLS/Grok.ts --web-only "<query>"       # web only
 *   bun ~/.claude/PAI/TOOLS/Grok.ts --x-only "<query>"         # X only
 *   bun ~/.claude/PAI/TOOLS/Grok.ts --code "<query>"           # + code execution
 *   bun ~/.claude/PAI/TOOLS/Grok.ts --json "<query>"
 *   bun ~/.claude/PAI/TOOLS/Grok.ts --model grok-4.3 "<query>"
 *
 * Options:
...
// Load environment — mirrors PAI/TOOLS/YouTubeApi.ts convention
...
async function main() {
  ...
  if (!query) {
    ...
    console.error(`Usage: bun ~/.claude/PAI/TOOLS/Grok.ts [--x-only|--web-only] [--model <id>] [--json] "<query>"`)
    process.exit(1)
  }
