 ...
 *
 * Usage:
 *   bun PAI/TOOLS/HealthSync.ts pull
 *   bun PAI/TOOLS/HealthSync.ts pull --source oura
 *   bun PAI/TOOLS/HealthSync.ts status
 *   bun PAI/TOOLS/HealthSync.ts current
 *   bun PAI/TOOLS/HealthSync.ts auth oura
 */
...
const CURRENT_PATH = join(HOME, ".claude", "PAI", "USER", "HEALTH", "current.json");
...
function usage(): string {
  return [
    "Usage:",
    "  bun PAI/TOOLS/HealthSync.ts pull [--source oura|eightsleep|apple|function]",
    "  bun PAI/TOOLS/HealthSync.ts status",
    "  bun PAI/TOOLS/HealthSync.ts current",
    "  bun PAI/TOOLS/HealthSync.ts auth oura",
  ].join("\n");
...
async function runStatus(): Promise<number> {
  ...
  for (const source of SOURCE_NAMES) {
    ...
    if (notConfigured) {
      log(`${source}: ${result.status}; fresh=n/a (not set up — see PAI/USER/HEALTH/SHORTCUT_SETUP.md)`);
      continue;
    }
...
async function exchangeOuraCode(ctx: Ctx, code: string): Promise<TokenStore> {
  ...
  const response = await timedFetch(
    "https://api.ouraring.com/oauth/token",
    {
      method: "POST",
      headers: {
        "Content-Type": "<REDACTED:high-entropy>",
      },
      body,
    },
    FETCH_TIMEOUT_MS,
