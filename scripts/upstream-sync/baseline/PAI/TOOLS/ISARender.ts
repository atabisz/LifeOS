...
const TOOLS_DIR = resolve(HOME, ".claude/PAI/TOOLS");
const TEMPLATE_HTML = join(TOOLS_DIR, "ISARender/template.html");
const TEMPLATE_CSS = join(TOOLS_DIR, "ISARender/template.css");
// Brand logo: user override via PAI_BRAND_LOGO_PATH env var (absolute path),
// else system default under PAI/ASSETS/, else inert (empty src).
const BRAND_LOGO_PATH_OVERRIDE = process.env.PAI_BRAND_LOGO_PATH ?? "";
const BRAND_LOGO_PATH_DEFAULT = resolve(HOME, ".claude/PAI/ASSETS/pai-logo.png");
const WORK_DIR = resolve(HOME, ".claude/PAI/MEMORY/WORK");
const WORK_JSON = resolve(HOME, ".claude/PAI/MEMORY/STATE/work.json");
