 ...
 *
 *   bun ~/.claude/PAI/TOOLS/InstallDerivedSync.ts             # install
 *   bun ~/.claude/PAI/TOOLS/InstallDerivedSync.ts --uninstall # remove
 *   bun ~/.claude/PAI/TOOLS/InstallDerivedSync.ts --status    # check
 */
...
async function install(): Promise<void> {
  ...
  const userDir = realpathSync(join(HOME, ".claude", "PAI", "USER"));
