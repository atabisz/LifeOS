 ...
 *   - CLI:  bun PAI/TOOLS/MemoryHealthCheck.ts
 */
...
const TOOLS_DIR = join(CLAUDE, "PAI/TOOLS");
const OBS_DIR = join(CLAUDE, "PAI/MEMORY/OBSERVABILITY");
...
{
  const HEARTBEAT_FILE = join(CLAUDE, "<REDACTED:high-entropy>");
...
const PRINCIPAL_MEM = join(CLAUDE, "PAI/USER/PRINCIPAL/PRINCIPAL_MEMORY.md");
const DA_MEM = join(CLAUDE, "PAI/USER/DIGITAL_ASSISTANT/DA_MEMORY.md");
