 ...
 *
 * Usage:
 *   bun PAI/TOOLS/MemoryInsights.ts            # default --days 1
 *   bun PAI/TOOLS/MemoryInsights.ts --days 7   # last week
 *
...
const OBS = pathResolve(ROOT, "PAI/MEMORY/OBSERVABILITY");
const PRINCIPAL_MEM = pathResolve(ROOT, "PAI/USER/PRINCIPAL/PRINCIPAL_MEMORY.md");
const DA_MEM = pathResolve(ROOT, "PAI/USER/DIGITAL_ASSISTANT/DA_MEMORY.md");
...
function parseArgs(): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === "--days" && i + 1 < argv.length) {
      ...
      console.log("usage: bun PAI/TOOLS/MemoryInsights.ts [--days N]");
