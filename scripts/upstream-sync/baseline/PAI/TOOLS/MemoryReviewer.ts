...
const RUNS_LOG_PATH = pathResolve(CLAUDE_ROOT, "PAI/MEMORY/OBSERVABILITY/reviewer-runs.jsonl");
const RUNS_DEBUG_DIR = pathResolve(CLAUDE_ROOT, "<REDACTED:high-entropy>");
const REVIEW_CONFIG_PATH = pathResolve(CLAUDE_ROOT, "PAI/USER/CONFIG/memory-review.json");
...
- identity → PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md OR PAI/USER/DIGITAL_ASSISTANT/DA_IDENTITY.md
...
- style → PAI/USER/PRINCIPAL/WRITINGSTYLE.md
...
- definition → PAI/USER/DEFINITIONS.md
...
- canonical-content → PAI/USER/CANONICAL_CONTENT.md
...
- resume → PAI/USER/PRINCIPAL/RESUME.md
...
- operational-rule → PAI/USER/CONFIG/OPERATIONAL_RULES.md
...
const PRINCIPAL_MEMORY_PATH = pathResolve(CLAUDE_ROOT, "PAI/USER/PRINCIPAL/PRINCIPAL_MEMORY.md");
const DA_MEMORY_PATH = pathResolve(CLAUDE_ROOT, "PAI/USER/DIGITAL_ASSISTANT/DA_MEMORY.md");
...
async function smokeTest(): Promise<number> {
  ...
  const mockResponse = JSON.stringify({
    items: [
      ...
      { type: "proposal", target_file: pathJoin(homedir(), ".claude/PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md"), edit: "RULE: E2E mock", confidence: 0.5, rationale: "smoke" },
    ],
  });

  // Use a synthetic transcript so we don't depend on real harness state
  const synthDir = pathJoin(CLAUDE_ROOT, "<REDACTED:high-entropy>");
  ...
  try {
    ...
    const PRINCIPAL_MEMORY_PATH = pathJoin(CLAUDE_ROOT, "PAI/USER/PRINCIPAL/PRINCIPAL_MEMORY.md");
