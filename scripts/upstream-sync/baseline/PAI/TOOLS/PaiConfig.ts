 ...
 * USER data (the actual values, sourced from PAI/USER/CONFIG/PAI_CONFIG.toml).
 *
 ...
 * any file under PAI/USER/ for these values — the path-rooting happens here.
 *
 ...
 *
 * See: PAI/DOCUMENTATION/SystemUserBoundary.md § "The four allowed access patterns".
 */
...
const DEFAULT_CONFIG_PATH = resolve(DEFAULT_HOME, ".claude/PAI/USER/CONFIG/PAI_CONFIG.toml");
...
export function loadPaiConfig(opts: { path?: string; force?: boolean } = {}): PaiConfig {
  const path = opts.path ?? process.env.PAI_CONFIG_PATH ?? DEFAULT_CONFIG_PATH;

  if (!existsSync(path)) {
    throw new Error(
      `PaiConfig: config file not found at ${path}. ` +
        `Create it (see PAI/USER/CONFIG/README.md) or set PAI_CONFIG_PATH.`,
    );
  }
...
export function paiUserDir(): string {
  try {
    ...
    return resolve(DEFAULT_HOME, ".claude/PAI/USER");
  }
}
...
function validateAndNormalize(raw: unknown, path: string): PaiConfig {
  ...
  return {
    ...
    paths: {
      userDir: expandHome(
        root.paths?.userDir ?? root.paths?.user_dir ?? resolve(DEFAULT_HOME, ".claude/PAI/USER"),
      ),
      memoryDir: expandHome(
        root.paths?.memoryDir ?? root.paths?.memory_dir ?? resolve(DEFAULT_HOME, ".claude/PAI/MEMORY"),
      ),
