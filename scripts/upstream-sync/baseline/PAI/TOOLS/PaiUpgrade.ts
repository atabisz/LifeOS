 ...
 *
 * Designed by: PAI/MEMORY/WORK/20260520-pai-system-user-separation-rebuild/PhaseG-design.md
 *
 * Usage:
 *   bun ~/.claude/PAI/TOOLS/PaiUpgrade.ts [--diagnose | --dry-run | --from-fresh-install] [--target=<version>]
 *
 * Exit codes:
...
const MIGRATIONS: Migration[] = [
  {
    ...
    isApplied: ({ claudeRoot }) =>
      ...
      existsSync(join(claudeRoot, "PAI/USER/CONFIG/settings.user.json")) &&
      existsSync(join(claudeRoot, "PAI/TOOLS/MergeSettings.ts")),
    apply: () => {
  ...
  },
  {
    ...
    isApplied: ({ claudeRoot }) => {
      ...
      const opRulesPresent = existsSync(join(claudeRoot, "PAI/USER/CONFIG/OPERATIONAL_RULES.md"));
  ...
  {
    id: "m-003",
    name: "Phase F — PaiConfig.ts present + PAI_CONFIG.toml populated",
    isApplied: ({ claudeRoot }) =>
      existsSync(join(claudeRoot, "PAI/TOOLS/PaiConfig.ts")) &&
      existsSync(join(claudeRoot, "PAI/USER/CONFIG/PAI_CONFIG.toml")),
    apply: () => {
      throw new Error("m-003 apply not implemented — PaiConfig.ts ships with public LifeOS; PAI_CONFIG.toml comes from `pai setup`. Missing artifact requires manual remediation.");
    },
  },
  {
    id: "m-004",
    name: "Phase G — PAI/USER is symlink to user data repo",
    isApplied: ({ claudeRoot }) => {
      const userPath = join(claudeRoot, "PAI/USER");
  ...
  {
    id: "m-005",
    name: "Phase G — PAI/MEMORY/ in .gitignore",
    isApplied: ({ claudeRoot }) => {
      const gitignore = join(claudeRoot, ".gitignore");
      if (!existsSync(gitignore)) return false;
      const content = readFileSync(gitignore, "utf8");
      // Already-applied if any PAI/MEMORY/ subpath is gitignored. The current
      // policy gitignores LEARNING/, OBSERVABILITY/, STATE/, etc. selectively;
      // Phase G may broaden this. For now, we detect "any PAI/MEMORY/ rule" as
      // applied.
...
function main(): never {
  ...
  if (!existsSync(join(CLAUDE_ROOT, "PAI"))) {
