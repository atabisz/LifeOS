#!/usr/bin/env bash
# PiSync.sh — bring ~/.pi/agent/ in line with current ~/.claude/PAI/
#
...
# ─── 1. PAI_SYSTEM_PROMPT.md ───────────────────────────
echo "[1/4] Sync PAI_SYSTEM_PROMPT.md"
awk 'NR==1 && /^---$/ {fm=1; next} fm && /^---$/ {fm=0; next} !fm' \
  "$PAI/PAI/PAI_SYSTEM_PROMPT.md" > "$PI/PAI_SYSTEM_PROMPT.md"
echo "  $(wc -l < "$PI/PAI_SYSTEM_PROMPT.md") lines"

# ─── 2. AGENTS.md ─────────────────────────────────────
echo "[2/4] Regenerate AGENTS.md"
ALG=$(tr -d '[:space:]' < "$PAI/PAI/ALGORITHM/LATEST")
{
  echo "# LifeOS on Pi"
  ...
  echo
  for f in \
    "$PAI/PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md" \
    "$PAI/PAI/USER/DIGITAL_ASSISTANT/DA_IDENTITY.md" \
    "$PAI/PAI/USER/PROJECTS.md" \
    "$PAI/PAI/USER/TELOS/PRINCIPAL_TELOS.md" \
    "$PAI/PAI/DOCUMENTATION/ARCHITECTURE_SUMMARY.md"; do
    [ -f "$f" ] || continue
...
echo "  Pi system prompt: $(wc -l < "$PI/PAI_SYSTEM_PROMPT.md" | tr -d ' ') lines"
