 ...
 *
 * Source of truth: PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md frontmatter (snake_case)
 * Mirror:         settings.json `.preferences` (camelCase, what consumers read)
 *
 * Consumers that read settings.json `.preferences`:
 *   - PAI_StatusLine.sh  → `.preferences.temperatureUnit`
 *
 * Idempotent: writes only when the mirrored block actually changed.
 *
 * Invocation:
 *   bun PAI/TOOLS/SyncIdentityToSettings.ts          # silent unless changed
 *   bun PAI/TOOLS/SyncIdentityToSettings.ts --verbose
 */
