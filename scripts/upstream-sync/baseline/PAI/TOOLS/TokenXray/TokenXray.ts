...
function loadLatestApiCostSnapshot(): ApiCostSnapshot | null {
  const path = join(homedir(), ".claude/PAI/MEMORY/OBSERVABILITY/anthropic-cost.jsonl");
  let raw: string;
  try {
    raw = readFileSync(path, "utf8");
  } catch {
...
function runActual(jsonOut: boolean): void {
  ...
  if (snap) {
    ...
    console.log(`  (no data — anthropic-cost.jsonl missing at PAI/MEMORY/OBSERVABILITY/)`);
  }
  console.log("");
...
Reads only ~/.claude/projects/*​/*.jsonl and PAI/MEMORY/OBSERVABILITY/anthropic-cost.jsonl.
Nothing leaves the machine.
`);
}
