#!/usr/bin/env bun
/**
 * UpdatePaiState — Writes PAI_STATE.json with per-dimension pct scores read by
 * the statusline (PAI/PAI_StatusLine.sh) STATE strip and the Pulse TELOS
 * dashboard rings.
 *
 * Pct semantics:
 ...
 *
 * Reads:  PAI/USER/TELOS/IDEAL_STATE/<DIM>.md (target articulation)
 *         PAI/USER/TELOS/CURRENT_STATE/<DIM>.md (actual coverage, when present)
 * Writes: PAI/USER/TELOS/PAI_STATE.json
 *
 ...
 *
 * Usage:
 *   bun ~/.claude/PAI/TOOLS/UpdatePaiState.ts
 *   bun ~/.claude/PAI/TOOLS/UpdatePaiState.ts --json
 */
...
function main(): void {
  ...
  if (process.argv.includes("--json")) {
    ...
    console.log(`PAI_STATE.json updated: ${STATE_FILE}`);
    for (const d of DIMENSIONS) {
