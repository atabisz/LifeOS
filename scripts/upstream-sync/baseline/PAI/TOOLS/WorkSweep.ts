 ...
 *
 *   bun ~/.claude/PAI/TOOLS/WorkSweep.ts            # apply
 *   bun ~/.claude/PAI/TOOLS/WorkSweep.ts --dry-run  # show diff only
 *   bun ~/.claude/PAI/TOOLS/WorkSweep.ts --since 48h  # widen the scan window
 *
 * What it does:
...
const PROJECTS_MD = join(PAI_DIR, "USER", "PROJECTS.md");
const PRINCIPAL_TELOS = join(PAI_DIR, "USER", "TELOS", "PRINCIPAL_TELOS.md");
...
const BPE_AUDIT_TARGETS = "force-loaded + doctrine surface: CLAUDE.md, PAI_SYSTEM_PROMPT.md, current ALGORITHM/v*.md";
...
async function sweepGoals(
  ...
  for (const g of goals) {
    ...
    const body = [
      ...
      "",
      `Source: \`PAI/USER/TELOS/PRINCIPAL_TELOS.md\` → \`## Active Goals\``,
      "",
      `---`,
      `*Auto-generated by LifeOS WorkSweep*`,
    ].join("\n");
...
async function sweepBpeCadence(
  ...
  const body = [
    ...
    `**Then stamp it:** \`bun ~/.claude/PAI/TOOLS/WorkSweep.ts --stamp-bpe\` to reset the clock.`,
    "",
