 ...
 * Path contract is duplicated in PAI/USER/HEALTH/SHORTCUT_SETUP.md — keep in sync.
 */
  ...
  if (!(await file.exists())) {
    ...
    if (!(await file.exists())) {
      return {
        ...
        lastSuccess: null,
        ms: Date.now() - startedAt,
        note: "no export yet — build the Shortcut per PAI/USER/HEALTH/SHORTCUT_SETUP.md",
      };
    }
  }
