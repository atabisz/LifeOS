...
async function refreshTokens(ctx: Ctx, tokens: TokenStore): Promise<TokenStore | null> {
  ...
  try {
    const response = await timedFetch(
      ...
      {
        method: "POST",
        headers: { "Content-Type": "<REDACTED:high-entropy>" },
        body,
      },
      FETCH_TIMEOUT_MS,
    );
...
export async function pull(ctx: Ctx): Promise<SourceResult> {
  ...
  if (tokens.oura === undefined) {
    return unconfigured("no Oura tokens — run: bun PAI/TOOLS/HealthSync.ts auth oura", startedAt);
  }
  ...
  if (oura.expires_at <= Math.floor(Date.now() / 1000) + REFRESH_SKEW_S) {
    ...
    if (refreshed === null || refreshed.oura === undefined) {
      return unconfigured("Oura token refresh failed — re-run: bun PAI/TOOLS/HealthSync.ts auth oura", startedAt);
    }
    tokens = refreshed;
    oura = refreshed.oura;
  }
