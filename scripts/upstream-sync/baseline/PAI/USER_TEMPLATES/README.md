...
1. Copy a template into `USER/` (without `_TEMPLATES/`):
   ```bash
   cp PAI/USER/_TEMPLATES/Books.md PAI/USER/Books.md
   ```
...
3. Or delete the template and write your own from scratch — Pulse only requires that the file exist where the manifest points (`PAI/PULSE/pages/<id>.manifest.toml`).
...
2. Add a `<id>.manifest.toml` under `PAI/PULSE/pages/` pointing at the new template's path.
3. Pick a kind (`collection`, `narrative`, `reference`, `index`) — that decides which UI component renders it.
4. Pick or write an adapter prompt under `PAI/PULSE/adapters/prompts/`.
5. Run `bun PAI/PULSE/Tools/ValidateManifests.ts` to confirm the new manifest is wired up.
