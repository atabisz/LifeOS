...
`PAI/USER/` holds everything specific to one principal. `CUSTOMIZATIONS/` is the part of USER that other system components read on every invocation — not standalone reference data (`CONTACTS.md`, `OPINIONS.md`) but live overlays that change how LifeOS behaves for this user.
