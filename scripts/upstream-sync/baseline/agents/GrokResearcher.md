...
bun ~/.claude/PAI/TOOLS/Grok.ts "<your research question>"

# web only (general/news, no social)
bun ~/.claude/PAI/TOOLS/Grok.ts --web-only "<query>"

# X only (pure social sentiment), structured output for parsing
bun ~/.claude/PAI/TOOLS/Grok.ts --x-only --json "contrarian takes on <topic> trending on X"

# add code execution (let Grok compute/analyze mid-search)
bun ~/.claude/PAI/TOOLS/Grok.ts --code "<query needing a calculation>"
```
