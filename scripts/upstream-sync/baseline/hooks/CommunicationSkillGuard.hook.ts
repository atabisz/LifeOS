 ...
 * `PAI_SKILL=_COMMUNICATION` (an inert env assignment the tools ignore). This
...
function isSkillRouted(command: string): boolean {
  return /PAI_SKILL=_COMMUNICATION\b/.test(command);
}
...
function main(): never {
  ...
  const lines = [
    ...
    "    3. The send command it builds carries  PAI_SKILL=_COMMUNICATION  and passes.",
    "",
    "  Explicit one-off override (logged): prefix the command with",
    "    PAI_SKILL=_COMMUNICATION  <your send command>",
