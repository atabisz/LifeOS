 ...
 *
 * State file: ~/.claude/PAI/MEMORY/STATE/isa-render-debounce/<session_id>.json
...
const STATE_DIR = join(homedir(), '.claude/PAI/MEMORY/STATE/isa-render-debounce');
const ISA_RENDER = join(homedir(), '.claude/PAI/TOOLS/ISARender.ts');
...
if (rendered.length || skipped.length) {
  try {
    ...
    const logDir = join(homedir(), '.claude/PAI/MEMORY/OBSERVABILITY');
