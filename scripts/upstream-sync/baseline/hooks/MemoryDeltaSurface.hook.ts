...
const WRITES_LOG = pathResolve(CLAUDE_ROOT, "PAI/MEMORY/OBSERVABILITY/memory-writes.jsonl");
const CURSOR = pathResolve(CLAUDE_ROOT, "PAI/MEMORY/OBSERVABILITY/memory-delta-cursor.json");
const HEALTH_LOG = pathResolve(CLAUDE_ROOT, "PAI/MEMORY/OBSERVABILITY/memory-health.jsonl");
const FRESHNESS_CACHE = pathResolve(CLAUDE_ROOT, "PAI/USER/CACHE/freshness.json");
const HEARTBEAT = pathResolve(CLAUDE_ROOT, "<REDACTED:high-entropy>");
...
function criticalHealthLine(): string | null {
  try {
    ...
    const blockers = (last.findings ?? [])
      ...
      .map((f: any) => f.message)
      .slice(0, 3);
    return `🩺 MEMORY HEALTH: CRITICAL — ${blockers.join(" · ") || `${last.counts?.critical ?? "?"} blocker(s)`}. Fix: bun ~/.claude/PAI/TOOLS/MemoryHealthCheck.ts`;
  } catch {
    return null;
  }
}
