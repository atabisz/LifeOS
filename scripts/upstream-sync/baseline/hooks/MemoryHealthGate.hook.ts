...
const CHECK = join(HOME, ".claude/PAI/TOOLS/MemoryHealthCheck.ts");
...
try {
  ...
  if (report.overall === "critical") {
    console.error(`🚨 Memory health: CRITICAL — ${report.counts.critical} blocker(s). Run: bun ~/.claude/PAI/TOOLS/MemoryHealthCheck.ts`);
