 ...
 * by promoting `PAI_SYSTEM_PROMPT.md § Output Format` from doctrine to structural
 * enforcement.
 *
 * TRIGGER: Stop
 *
...
import { collectCurrentResponseText } from "../PAI/TOOLS/TranscriptParser";
import { scanAiSpeak } from "./lib/ai-speak-patterns";
import { readFileSync, appendFileSync, mkdirSync } from "fs";
import { dirname, join } from "path";

const OBS_PATH = join(
  process.env.PAI_DIR || join(process.env.HOME!, ".claude", "LIFEOS"),
  "MEMORY",
  "OBSERVABILITY",
  "format-gate.jsonl",
);

// Multiline-anchored unicode banners — see PAI_SYSTEM_PROMPT.md § Mode Templates.
...
async function main(): Promise<void> {
  ...
  if (matchedIndex === -1) {
    ...
    // PAI_SYSTEM_PROMPT.md § Output Format + the non-blocking DriftReminder nudge.
    appendObs({
      ts: new Date().toISOString(),
    ...
    console.log(
      JSON.stringify({
        ...
        reason:
          ...
          "See PAI_SYSTEM_PROMPT.md § 🧠 MEMORY indicator. Rewrite this turn with the heartbeat line.",
      }),
    );
    process.exit(0);
  }
