...
5. **The model is the security boundary**: Constitutional Security Protocol in `PAI_SYSTEM_PROMPT.md` + native `permissions.deny` in `settings.json`. Hooks don't enforce — they tag.
...
3. Tail observability: `tail -f ~/.claude/PAI/MEMORY/OBSERVABILITY/permission-decisions.jsonl`
...
Pre-state tag: `pre-bpe-cuts-2026-05-06`. Restoration: see `PAI/MEMORY/WORK/20260506-comprehensive-hook-bpe-audit/RESTORATION.md`.
...
- `PAI/USER/SECURITY/{PATTERNS.yaml, ...}` plus 8 of 9 `PAI/DOCUMENTATION/Security/*.md`
