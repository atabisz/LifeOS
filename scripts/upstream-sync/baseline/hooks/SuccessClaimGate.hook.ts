 ...
 * Doctrinal anchor: PAI_SYSTEM_PROMPT.md § Verification + Algorithm v6.6.0 § INLINE
 ...
 *     PAI_SYSTEM_PROMPT.md already bans them; not this hook's job to enforce)
...
// Doctrine (PAI_SYSTEM_PROMPT.md § Verification): curl is NOT verification for
...
async function main(): Promise<void> {
  ...
  if (isUnverifiedWebUiClaim(message)) {
    appendObs({
      ...
      detected_claim: "<REDACTED:high-entropy>",
      evidence_found: false,
  ...
  if (!evidence) {
    ...
    // "replace"). Verification discipline stays doctrine (PAI_SYSTEM_PROMPT.md
