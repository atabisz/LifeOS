...
const LOG_PATH = join(HOME, ".claude/PAI/MEMORY/OBSERVABILITY/system-file-guard.jsonl");
...
function denyMessage(relPath: string, pattern: string, match: string): string {
  return [
    ...
    "",
    "    import { loadPaiConfig, paiUserDir } from '~/.claude/PAI/TOOLS/PaiConfig';",
    "",
    "  Canonical USER zones (any of these can hold the content):",
    "    PAI/USER/PRINCIPAL/        principal identity files",
    "    PAI/USER/DIGITAL_ASSISTANT/  DA identity / voice / personality",
    "    PAI/USER/TELOS/            mission / goals / strategies",
    "    PAI/USER/CONFIG/           settings.user.json, PAI_CONFIG.toml",
    "    PAI/USER/CUSTOMIZATIONS/   personal skills / arbol overrides",
    "    PAI/MEMORY/                work sessions, learnings, observability",
    "",
    "  Doctrine: PAI/DOCUMENTATION/SystemUserBoundary.md",
    "",
    "═══════════════════════════════════════════════════════════════════════════",
    "",
  ].join("\n");
