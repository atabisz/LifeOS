...
const OBS_PATH = join(PAI_DIR, "MEMORY", "OBSERVABILITY", "writing-gate.jsonl");
const RUNS_PATH = join(PAI_DIR, "MEMORY", "OBSERVABILITY", "pangram-runs.jsonl");
...
const BLOCK_REASON =
  ...
  "(2) run `bun ~/.claude/PAI/TOOLS/PangramScore.ts --file <draft>` on the ACTUAL draft text so the run is " +
