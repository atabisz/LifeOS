...
export const CONTAINMENT_ZONES: readonly ContainmentZone[] = [
  ...
  },
  {
    ...
    description: "Pre-sanitization snapshots of SYSTEM files (e.g. PAI_SYSTEM_PROMPT pre-restructure copies). Legitimately retain identity literals from before sanitization; never ship publicly. Already excluded from staging by RSYNC_EXCLUDES — this zone makes DenyListCheck classify them as private-zone rather than real-leak.",
  },
  {
...
export const PATTERN_ALLOWLIST_FILES: readonly string[] = [
  ...
  // NOTE (Phase G.6, 2026-05-25 after Forge iter-5): `PAI/TOOLS/PaiUpgrade.ts`
  // and `PAI/PULSE/Tools/ReleaseAudit.ts` were REMOVED from this allowlist.
