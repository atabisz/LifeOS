 ...
 *   - PAI/USER/DIGITAL_ASSISTANT/DA_IDENTITY.md (canonical DA identity)
 *   - PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md  (canonical Principal identity)
 *
...
import { loadPaiConfig } from '../../PAI/TOOLS/PaiConfig';

const HOME = process.env.HOME!;
const SETTINGS_PATH = join(HOME, '.claude/settings.json');

// Identity-file paths derive from PaiConfig's userDir. On fresh installs where
// PAI_CONFIG.toml hasn't been created yet, fall back to the conventional
// PAI/USER/ location so identity loading still bootstraps. Lazy try/catch so
// a malformed PaiConfig never breaks identity bootstrap.
function paiUserDir(): string {
  try {
    return loadPaiConfig().paths.userDir;
  } catch {
    return join(HOME, '.claude/PAI/USER');
  }
}
