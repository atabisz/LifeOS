 ...
 * - PAI_DIR (~/.claude/LIFEOS) — LifeOS data: MEMORY, Algorithm, Tools, USER
 ...
 *   2. PAI_DIR env var (expanded)
 *   3. ~/.claude/LIFEOS  (live default — byte-identical to pre-plugin behavior)
 *
 * The CLAUDE_PLUGIN_ROOT guard MUST precede the PAI_DIR check: in a packed
 * plugin, bin/pai exports PAI_DIR equal to CLAUDE_PLUGIN_ROOT (the flattened
 * claude-home root), so trusting PAI_DIR first would drop the trailing /PAI
 * segment and mis-resolve paiPath() to ROOT/MEMORY instead of ROOT/PAI/MEMORY.
 * Resolving via getClaudeDir() + 'PAI' keeps the live ~/.claude/LIFEOS →
 * plugin ${ROOT}/PAI mapping that the packer's ~/.claude/ → ${PAI_DIR} rewrite assumes.
 */
