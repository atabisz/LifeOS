 ...
 *
 * Repo identity comes from `PAI/USER/WORK/work_repo.json` (privacy-attested,
 * gh-verified at write time by `skills/_ULWORK/Tools/SetWorkRepo.ts`).
 * Kanban columns + poll interval come from `PAI/USER/WORK/config.yaml`
 ...
 *   - PAI/PULSE/modules/work.ts    — kanban renderer
 *
...
const REPO_JSON_PATH = join(PAI_DIR, "USER", "WORK", "work_repo.json");
const COLUMNS_YAML_PATH = join(PAI_DIR, "USER", "WORK", "config.yaml");
