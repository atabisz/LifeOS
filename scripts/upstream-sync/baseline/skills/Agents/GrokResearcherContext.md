...
```bash
bun ~/.claude/PAI/TOOLS/Grok.ts "<your research question>"          # web + X (default)
bun ~/.claude/PAI/TOOLS/Grok.ts --web-only "<query>"                 # general/news only
bun ~/.claude/PAI/TOOLS/Grok.ts --x-only --json "<social query>"     # X sentiment, parseable
bun ~/.claude/PAI/TOOLS/Grok.ts --code "<query needing math>"        # + code execution
```
