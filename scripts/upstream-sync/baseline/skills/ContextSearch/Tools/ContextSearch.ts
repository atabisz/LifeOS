...
const STATE_DIR = join(PAI_DIR, "PAI", "MEMORY", "STATE");
const WORK_DIR = join(PAI_DIR, "PAI", "MEMORY", "WORK");
...
const PROJECT_SLUG = PAI_DIR.replace(/[/.]/g, "-");
const JSONL_DIR = join(PAI_DIR, "Projects", PROJECT_SLUG);
...
function searchWorkJson(tokens: string[], since: Date | null, until: Date | null = null): Result[] {
  ...
  for (const [slug, entry] of Object.entries<any>(sessions)) {
    ...
    out.push({
      ...
      recencyDays: days,
      path: entry.isa ? join(PAI_DIR, entry.isa) : undefined,
    });
  }
  out.sort((a, b) => {
