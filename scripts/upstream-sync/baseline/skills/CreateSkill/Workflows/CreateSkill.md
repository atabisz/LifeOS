...
User-specific preferences, project names, domain lists, and war stories go in `~/.claude/PAI/USER/CUSTOMIZATIONS/SKILLS/<SkillName>/` — the skill body loads these at runtime via the Customization block. This keeps the public skill generic while each LifeOS user layers their own context.
