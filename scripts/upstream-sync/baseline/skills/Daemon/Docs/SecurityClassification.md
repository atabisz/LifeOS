...
| PAI/USER/TELOS/FINANCES/ | Financial data |
| PAI/USER/TELOS/HEALTH/ | Health data |
| PAI/USER/TELOS/TELOS.md `## Traumas` | Deeply personal |
...
The classification is config-driven, not hardcoded. The aggregator reads two user-zone config files in `PAI/USER/DAEMON/`: a public-projects allowlist (one project name or repo URL per line; empty by default) and a private-projects blocklist (same format; empty by default).
...
The aggregator reads named blocklists from a free-form per-user file in `PAI/USER/DAEMON/` (one name per line) AND from the user's contacts file in `PAI/USER/`. Every name in the contacts file is automatically blocked from public output.
...
User-specific additional path patterns can be added to a free-form per-user file in `PAI/USER/DAEMON/` (one path or glob per line).
...
Users customize this classification by placing overrides in `PAI/USER/DAEMON/`:

```
PAI/USER/DAEMON/public-projects.md     # opt-in projects
PAI/USER/DAEMON/private-projects.md    # opt-out projects
PAI/USER/DAEMON/blocked-names.md       # additional names to scrub
PAI/USER/DAEMON/blocked-paths.md       # additional paths to scrub
PAI/USER/DAEMON/SecurityOverrides.md   # free-form additional rules
```
