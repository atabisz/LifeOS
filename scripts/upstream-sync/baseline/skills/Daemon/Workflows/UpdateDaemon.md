...
```bash
bun ${PAI_SKILL_DIR}/Tools/DaemonAggregator.ts --preview --verbose
```
...
```bash
bun ${PAI_SKILL_DIR}/Tools/DaemonAggregator.ts --diff ${PAI_USER_DIR}/Daemon/daemon.md
```
...
```bash
bun ${PAI_SKILL_DIR}/Tools/DaemonAggregator.ts --output ${PAI_USER_DIR}/Daemon/daemon.md
```
...
```bash
cd ${PAI_SKILL_DIR}/Mcp && bun install && bun update-daemon
```
