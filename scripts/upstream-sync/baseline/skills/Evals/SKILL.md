...
`~/.claude/PAI/USER/CUSTOMIZATIONS/SKILLS/Evals/`
...
```bash
# Run an eval suite
bun run ${PAI_SKILL_DIR}/Tools/AlgorithmBridge.ts -s <suite>

# Log a failure for later conversion
bun run ${PAI_SKILL_DIR}/Tools/FailureToTask.ts log "description" -c category -s severity

# Convert failures to test tasks
bun run ${PAI_SKILL_DIR}/Tools/FailureToTask.ts convert-all

# Manage suites
bun run ${PAI_SKILL_DIR}/Tools/SuiteManager.ts create <name> -t capability -d "description"
bun run ${PAI_SKILL_DIR}/Tools/SuiteManager.ts list
bun run ${PAI_SKILL_DIR}/Tools/SuiteManager.ts check-saturation <name>
bun run ${PAI_SKILL_DIR}/Tools/SuiteManager.ts graduate <name>
```
...
bun run ${PAI_SKILL_DIR}/Tools/AlgorithmBridge.ts -s regression-core -r 3 -u
```
