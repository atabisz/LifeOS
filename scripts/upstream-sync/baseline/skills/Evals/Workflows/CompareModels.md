...
Each run's `results.json` lands at `~/.claude/PAI/MEMORY/STATE/Evals-Results/<use-case>-<model>/<run-id>/results.json`. Side-by-side comparison is done by reading those JSONs (`jq`) — there is no built-in cross-model comparison CLI in this skill.
...
bun run ~/.claude/Templates/Tools/RenderTemplate.ts \
  -t Evals/Report.hbs \
  -d ~/.claude/PAI/MEMORY/STATE/Evals-Results/<use-case>/models/<run-id>/summary.yaml \
  -o ~/.claude/PAI/MEMORY/STATE/Evals-Results/<use-case>/models/<run-id>/report.md
```
