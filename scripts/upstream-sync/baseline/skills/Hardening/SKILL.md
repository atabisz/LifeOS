...
- **Testing Doctrine Rule #11** — names fast-check as the property-testing primitive (`PAI/DOCUMENTATION/Testing/TestingDoctrine.md`).
- **ISA format** — new `bun-property` ISC type with `property | generator | runs` columns (`PAI/DOCUMENTATION/ISA/IsaFormat.md` § ISC Type Vocabulary).
- **Algorithm v6.10.0 candidate** — VERIFY-phase Hardening gate at E4/E5; `PropertyAudit` as capability #20 (`PAI/ALGORITHM/v6.10.0.md`).
- **System prompt** — Verification Is the Mechanism section at top (`PAI/PAI_SYSTEM_PROMPT.md`).
