...
| **Phase0Setup** | fresh install; DA name still reads "PAI"; PRINCIPAL_IDENTITY still reads "User"; PROJECTS sample-row only; `.env` missing required keys | `Workflows/Phase0Setup.md` |
...
```bash
bun ~/.claude/PAI/TOOLS/InterviewScan.ts --json | jq '[.targets[] | select(.phase == 0 and .completeness_score < 80)] | length'
```
...
- Fresh install (DA name still "PAI", PROJECTS sample-row only) → Phase0Setup first, then ContextCheckin.
