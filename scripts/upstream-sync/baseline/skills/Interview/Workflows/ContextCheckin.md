...
For fresh installs (DA name still "PAI", placeholder identity, sample-row PROJECTS), route to **Phase0Setup** instead.
...
```bash
bun ~/.claude/PAI/TOOLS/TelosFreshness.ts --json          # per-section TELOS
bun -e "import { readContextFreshness } from '$HOME/.claude/PAI/TOOLS/TelosFreshness'; console.log(JSON.stringify(readContextFreshness(), null, 2))"
```
...
If `readContextFreshness()` reports a file with `why: "no frontmatter"`, the migration hasn't been run on that file. Stop and prompt: *"<file> doesn't have the freshness convention yet. Want me to run `bun ~/.claude/PAI/TOOLS/MigrateContextFreshness.ts` first?"*
...
- **PAI_SYSTEM_PROMPT.md** — constitutional rules: *"System prompt at {N}d. Want to review the operational rules section, or is anything constitutional pending?"* (for this file especially, default to surfacing rather than editing — this is the most load-bearing file in the system).
...
bun ~/.claude/PAI/TOOLS/TelosFreshness.ts --bump <slug>

# Constitutional file — review marker (NOT bumpContextTimestamp; that's for writes)
bun -e "import { bumpReviewedTimestamp } from '$HOME/.claude/PAI/TOOLS/TelosFreshness'; console.log(bumpReviewedTimestamp('<absolute-path>', 'user'))"
```
...
```bash
bun ~/.claude/PAI/TOOLS/TelosFreshness.ts        # final TELOS state
bun ~/.claude/PAI/TOOLS/ContextAudit.ts          # content quality findings (read-only)
```
...
bun ~/.claude/PAI/TOOLS/ArchitectureSummaryGenerator.ts generate 2>/dev/null || true
```
...
Read PAI/PAI_SYSTEM_PROMPT.md fully
```

> "I read your context. **PAI_SYSTEM_PROMPT** hasn't been touched in **112d** (threshold 90d). The Hard Prohibitions section says: *'Never self-rate responses or add unsolicited ratings. Never modify working features unprompted. Analysis means read-only…'* Still want all three of those, or has anything changed in how you want me to operate?"
...
- **Migration not run yet.** First call to `readContextFreshness()` returns one or more files with `why: "no frontmatter"`. Run `bun ~/.claude/PAI/TOOLS/MigrateContextFreshness.ts` once before continuing.
