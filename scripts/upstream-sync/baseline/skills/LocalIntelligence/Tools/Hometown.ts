 ...
 *
 * Identity file location is configurable via env var `PAI_PRINCIPAL_IDENTITY`,
 * defaulting to `~/.claude/PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md`.
 *
...
export async function readHometown(
  identityPath: string = process.env.PAI_PRINCIPAL_IDENTITY ?? IDENTITY_DEFAULT
): Promise<Hometown> {
