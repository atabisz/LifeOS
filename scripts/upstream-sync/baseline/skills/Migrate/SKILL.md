...
No `Workflows/` directory — the single migration procedure runs inline through Phases 1–6 below, backed by two tools in `PAI/TOOLS/`.

| Trigger | Workflow | File |
|---------|----------|------|
| /migrate, migrate content, bulk import, import from other LifeOS, import CLAUDE.md / Cursor rules / Obsidian / Notion / Apple Notes export, bring in old notes | Inline Phases 1–6 (identify → scan → route → approve → UNCLEAR → summary) | `PAI/TOOLS/MigrateScan.ts` + `PAI/TOOLS/MigrateApprove.ts` |
