 ...
 * inlined, because Pulse's own installer (PAI/PULSE/setup.ts) is a bundled
...
function deployStatusline(ctx: Ctx): ComponentResult {
  ...
  const av = availability("PAI_StatusLine.sh", ctx);
  const scriptPath = join(ctx.lifeosDir, "PAI_StatusLine.sh");
  ...
  // the script under its own PAI/, and the old literal pointed at the wrong tree.
  const command = scriptPath.startsWith(`${ctx.home}/`)
    ? `$HOME/${scriptPath.slice(ctx.home.length + 1)}`
    : scriptPath;

  if (!av.inLive && !av.inPayload) {
    r.blockers.push(`PAI_StatusLine.sh not in live tree (${scriptPath}) or payload`);
    return r;
  }
  r.ready = true;
  if (!ctx.apply) {
    if (!av.inLive) r.actions.push(`copy PAI_StatusLine.sh from payload → ${scriptPath}`);
    r.actions.push(`chmod +x ${scriptPath}`, `wire settings.json statusLine → ${command}`);
    return r;
  }

  try {
    ensurePresent("PAI_StatusLine.sh", ctx);
...
function main(): void {
  ...
  const ctx: Ctx = {
    ...
    payloadRoot: join(skillRoot, "install", "PAI"),
