 ...
 * `@PAI/DOCUMENTATION/ARCHITECTURE_SUMMARY.md` import in CLAUDE.md dangles.
 *
 ...
 *
 * Targets the config-root runtime at the ALL-CAPS `<configRoot>/PAI/` so it
 * matches the `@PAI/...` imports in CLAUDE.md (NOT mixed-case `LifeOS`).
 *
 * Usage:
...
//  - PAI_INSTALL legacy installer engine, not part of the live runtime
//  - node_modules / .git  never deploy
// copyMissing's own SKIP_DIRS covers MEMORY/PAI_INSTALL when nested, but these
// are TOP-LEVEL entries of the runtime payload, so we filter them here explicitly.
const RUNTIME_SKIP = new Set(["USER", "MEMORY", "PAI_INSTALL", "node_modules", ".git"]);
...
/** (b) runtime: install/LifeOS/<entry> → configRoot/PAI/<entry>, skipping RUNTIME_SKIP. */
function deployRuntime(payloadInstall: string, configRoot: string, apply: boolean): DeployResult {
  const src = join(payloadInstall, "PAI");
