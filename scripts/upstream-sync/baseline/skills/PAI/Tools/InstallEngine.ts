 ...
 * `PAI/PAI_INSTALL/engine/` (detect.ts + the relevant types), reshaped for the
...
const SKIP_DIRS = new Set(["node_modules", ".git", "PAI_INSTALL", "MEMORY"]);
 ...
 * Atomic per-file (tmp + rename). Skips node_modules/.git/PAI_INSTALL/MEMORY.
...
/**
 * Establish the system/user separation contract: the live `<configRoot>/PAI/USER`
...
/**
 * Validate the symlink contract: `<configRoot>/PAI/USER` is a symlink → `<configDir>/USER`.
 ...
 * `${PAI_DIR}/x`, `$PAI_DIR/x`, or `~/.claude/x` dedupes to one.
 */
function normalizeCommand(cmd: string): string {
  return cmd
    .replace(/\$\{?PAI_DIR\}?|\$\{?CLAUDE_PROJECT_DIR\}?|\$\{?CLAUDE_PLUGIN_ROOT\}?|~\/\.claude|\$HOME\/\.claude|\$\{HOME\}\/\.claude/g, "§ROOT§")
    .replace(/\s+/g, " ")
    .trim();
}
 ...
 * `<!-- @PAI/USER/... -->` are activated to `@PAI/USER/...` only when the target
...
export function activateImports(claudeMdPath: string, configRoot: string): { activated: string[]; skipped: string[] } {
  ...
  // Two dormant-import conventions: the public CLAUDE.md ships `# @PAI/USER/...`
  // (hash-prefixed so the @ isn't at line-start and Claude Code skips it); the
  // older form is `<!-- @PAI/USER/... -->`. Activation strips the prefix so the
  // import sits at line-start and resolves.
  const commented = /^\s*#\s+(@[\w./-]+)\s*$|^\s*<!--\s*(@[\w./-]+)\s*-->\s*$/;
  const out = lines.map((line) => {
    const m = line.match(commented);
    if (!m) return line;
    const importPath = m[1] || m[2]; // e.g. @PAI/USER/TELOS/PRINCIPAL_TELOS.md
