 ...
 * (PRINCIPAL_TELOS.md, PAI_STATE.json) via the shipped PAI/TOOLS generators.
...
function main(): void {
  ...
  const configDir = get("--config-dir") || process.env.PAI_CONFIG_DIR || join(home, ".config", "LIFEOS");
    ...
    try {
      execFileSync("bun", [join(toolsDir, g)], {
        ...
        env: {
          ...
          PAI_DIR: join(configRoot, "LIFEOS"),
          // GenerateTelosSummary resolves its TELOS dir via PaiConfig.paiUserDir(),
          // which reads PAI_CONFIG_PATH (NOT PAI_DIR). UpdatePaiState resolves via
          // PAI_DIR. Pass BOTH so both generators target the same install root —
          // otherwise a non-default config root mis-targets ~/.claude.
          PAI_CONFIG_PATH: join(configRoot, "LIFEOS", "USER", "CONFIG", "PAI_CONFIG.toml"),
        },
        timeout: 60000,
      });
      ran.push(g);
