# LifeOS {{PAI_VERSION}} — LifeOS (the Life Operating System)

> **LifeOS is the Life OS. The DA is the principal's AI assistant. Pulse is the Life Dashboard.**
> Canonical thesis: `PAI/DOCUMENTATION/LifeOs/LifeOsThesis.md`. Everyone running LifeOS names their own DA. LifeOS targets AS3 on the LifeOS Maturity Model, with lineage from "The Real Internet of Things" (2016).
...
# @PAI/USER/PRINCIPAL/PRINCIPAL_IDENTITY.md
# @PAI/USER/DIGITAL_ASSISTANT/DA_IDENTITY.md
# @PAI/USER/PROJECTS.md
# @PAI/USER/CONFIG/OPERATIONAL_RULES.md

## Constitutional layer

Constitutional rules, mode architecture, mode templates (NATIVE / ALGORITHM / MINIMAL field structures), effort overrides (`/e1`–`/e5`), verification doctrine, hard prohibitions, security protocol, and operational rules all live in the system prompt: `PAI/PAI_SYSTEM_PROMPT.md`. When this file and the system prompt disagree, the system prompt wins.
...
## LifeOS System (paths under `PAI/DOCUMENTATION/` unless noted)
...
- **System prompt** — `PAI/PAI_SYSTEM_PROMPT.md` (loaded via `--append-system-prompt-file`; home of mode templates and constitutional rules)
...
## Principal — Identity & Voice (paths under `PAI/USER/`)
...
- TELOS (single source of truth, unified H2 sections) — `PAI/USER/TELOS/TELOS.md`
- Auto-generated derivative — `PAI/USER/TELOS/PRINCIPAL_TELOS.md`
- Dimension percentages — `PAI/USER/TELOS/PAI_STATE.json` (Pulse rings + statusline read from this)
- Freshness convention — see `PAI/DOCUMENTATION/Freshness/FreshnessSystem.md`

## Principal — Work (paths under `PAI/USER/`)
