...
#     3. Fetches the pinned LifeOS release (or uses $PAI_SRC locally)
...
#     PAI_SRC=/path/to/PAI_RELEASES/<version> bash install.sh
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail

# ─── Pinned release ──────────────────────────────────────────────
PAI_VERSION="${PAI_VERSION:-6.0.0}"
PAI_TAG="v${PAI_VERSION}"
# Repo owner/name are parameterized — set at publish time, never hard-coded here.
PAI_REPO="${PAI_REPO:-danielmiessler/LifeOS}"
PAI_TARBALL_URL="${PAI_TARBALL_URL:-https://github.com/${PAI_REPO}/archive/refs/tags/${PAI_TAG}.tar.gz}"
# Where the LifeOS skill dir lives inside the release tree:
PAI_RELEASE_SUBPATH="${PAI_RELEASE_SUBPATH:-LifeOS}"
# Local source override — point at a PAI_RELEASES/<version> dir to install offline.
PAI_SRC="${PAI_SRC:-}"
# Target skills dir (auto-detected below; override to force).
PAI_SKILLS_DIR="${PAI_SKILLS_DIR:-}"
DRY_RUN="${DRY_RUN:-0}"
...
if [ -z "$PAI_SKILLS_DIR" ]; then
  if [ -d "$HOME/.claude" ]; then PAI_SKILLS_DIR="$HOME/.claude/skills"
  elif [ -d "$HOME/.config/claude" ]; then PAI_SKILLS_DIR="$HOME/.config/claude/skills"
  else PAI_SKILLS_DIR="$HOME/.claude/skills"; fi
fi
info "Skills dir: ${BOLD}${PAI_SKILLS_DIR/#$HOME/~}${RESET}"
TARGET="$PAI_SKILLS_DIR/LifeOS"
if [ -e "$TARGET" ]; then
  TS="$(date +%Y%m%d-%H%M%S)"
...
step "3/5  Fetching LifeOS ${PAI_TAG}"
TMP_DIR="$(mktemp -d -t lifeos-install-XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT
if [ -n "$PAI_SRC" ]; then
  info "Local source: ${PAI_SRC/#$HOME/~}"
  SRC_SKILL="$PAI_SRC/$PAI_RELEASE_SUBPATH"
  [ -d "$SRC_SKILL" ] || { error "LifeOS skill not found at $SRC_SKILL"; exit 1; }
else
  info "Downloading ${PAI_TAG} (HTTPS, no auth)..."
  if [ "$PAI_REPO" = "OWNER/REPO" ]; then
    error "Network install needs PAI_REPO set (owner/name), or use PAI_SRC for a local install."; exit 1
  fi
  run bash -c "curl -fsSL '$PAI_TARBALL_URL' | tar -xzf - -C '$TMP_DIR'"
  EXTRACTED="$(find "$TMP_DIR" -mindepth 1 -maxdepth 1 -type d | head -n 1)"
  SRC_SKILL="$EXTRACTED/$PAI_RELEASE_SUBPATH"
  [ -d "$SRC_SKILL" ] || { error "LifeOS skill not in tarball at $PAI_RELEASE_SUBPATH"; exit 1; }
fi
success "Fetched ${PAI_TAG}"

# ─── Step 4: Place the skill (additive) ──────────────────────────
step "4/5  Installing the LifeOS skill (additive — nothing else touched)"
run mkdir -p "$PAI_SKILLS_DIR"
