...
WORK_DIR=$(jq -r '.sessions | to_entries | map(select(.value.phase != "complete" and .value.mode != "native" and .value.mode != "starting")) | sort_by(.value.updatedAt) | reverse | .[0].key // empty' ~/.claude/PAI/MEMORY/STATE/work.json)
